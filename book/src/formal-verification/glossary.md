...
<div class="g"><div class="g-head"><span class="term">DL reduction bound</span><span class="anchor">AGM.Probability.TextbookDLAdvantageLE · AGM.StraightLineFiniteSecurity.StraightLineDirectDlogProfile · Action.AdaptiveStatementProfile.AdaptiveStatementCachedDirectDlogProfile</span></div><div class="def">The programmed-basis reduction turns a computed relation into either a discrete-log solution or one <code>1/|Fp|</code> miss hyperplane. The straight-line and adaptive profiles record random-oracle queries, group work, and direct-decode work separately. The adaptive-statement cached profile retains one <code>runWithAnnotations</code> result and charges all provenance scans, comparisons, decoding, and programmed-basis work to an explicit post-processing budget. Thus a <code>2^123</code> target maps to at most <code>2^124</code> random-oracle queries and group work, while the consensus-generic statistical remainder stays <code>2^-83</code>; the older <code>2^127</code> eight-traversal profile remains a conservative compatibility theorem. Lean proves this resource arithmetic but deliberately leaves the finite-security Vesta DLOG advantage at the stated resource ceiling as a caller-supplied cryptographic premise.</div></div>
</section>
...
<div class="g"><div class="g-head"><span class="term">capstones</span><span class="anchor">Fixtures/MultiAction/Honest/ActionCapstone.lean</span></div><div class="def">The top-level results a route ends at. The consensus-generic adaptive-statement knowledge capstone lets the adversary output the public statement and proof together, binds the VK and selected instance commitments before <code>theta</code>, returns Type-valued private witnesses or relation data, and bounds acceptance when extraction fails by profiled DLOG advantage plus the statistical remainder. Its cached profile gives <code>Adv_DLOG(2^124, 2^124) + 2^-83</code> at the <code>2^123</code> work target; the ordinary fixed-statement, conservative eight-traversal, and sequential endpoints remain explicit compatibility variants.</div></div>
