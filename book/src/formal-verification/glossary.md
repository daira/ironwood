...
<div class="g"><div class="g-head"><span class="term">DL reduction bound</span><span class="anchor">AGM.Probability.TextbookDLAdvantageLE · AGM.CostedOracle.CostedLabeledOracleComp · Action.AdaptiveStatementCost.CertifiedAdaptiveStatementDlogProfile</span></div><div class="def">The programmed-basis reduction turns a computed relation into either a discrete-log solution or one <code>1/|Fp|</code> miss hyperplane. The adaptive route uses a costed oracle language whose erasure is the original algebraic adversary, reifies Vesta operations, and derives reduction work from a complete shape-level trace. One cached execution supplies the finder and witness extractor. At <code>Q ≤ 2^123</code>, the certified endpoints bound failure by <code>Adv_DLOG(2^126, 2^126) + 2^-83</code> for either a <code>2^123</code> adversary-work budget or a <code>2^125</code> budget; in the latter case the cached adversary-plus-reduction group work is proved at most <code>2^126</code>, while oracle queries are separately proved at most <code>2^124</code>. These numbers are coverage parameters of the proved reduction, not a hardness premise or a claim that Lean computes Vesta's DLOG advantage. Giving that advantage term a concrete security interpretation remains outside the theorem.</div></div>
</section>
...
<div class="g"><div class="g-head"><span class="term">capstones</span><span class="anchor">Capstones/Action.lean</span></div><div class="def">The adaptive-statement capstones prove a knowledge-failure bound for the deployed Action verifier against represented online algebraic adversaries, with Fiat–Shamir challenge derivation modelled as a random oracle. The adversary chooses the public statement and proof together, and the selected instance prefix is bound before <code>theta</code>. For every consensus-valid Action count, a costed adversary bounded by either <code>2^123</code> or <code>2^125</code> group operations has failure probability at most <code>Adv_DLOG(2^126, 2^126) + 2^-83</code>, provided its independently stated oracle-query count is at most <code>2^123</code>. Erasure proves that cost instrumentation preserves the original game, and cached execution is proved equivalent to the existing finder and extraction-success event. The quoted powers are the resource coverage of the bound—not a universal failure ceiling—and the result does not cover arbitrary non-algebraic adversaries or identify concrete Blake2b with the modeled oracle.</div></div>
