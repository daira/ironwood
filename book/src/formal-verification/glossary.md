...
<div class="g"><div class="g-head"><span class="term">DL reduction bound</span><span class="anchor">AGM.Probability.TextbookDLAdvantageLE · AGM.StraightLineFiniteSecurity.StraightLineDirectDlogProfile · Action.AdaptiveStatementProfile.AdaptiveStatementDirectDlogProfile</span></div><div class="def">The programmed-basis reduction turns a computed relation into either a discrete-log solution or one <code>1/|Fp|</code> miss hyperplane. The straight-line and adaptive profiles record random-oracle queries, group work, and direct-decode work separately. The adaptive-statement implementation does not retain one global adversary execution across its complete eight-stage finder, so the profile charges all eight traversals conservatively. Thus a <code>2^123</code> target maps to at most <code>2^127</code> random-oracle queries and group work, while the consensus-generic statistical remainder stays <code>2^-83</code>. Lean proves this resource arithmetic but deliberately leaves the finite-security Vesta DLOG advantage at that resource ceiling as a caller-supplied cryptographic premise.</div></div>
</section>
...
<div class="g"><div class="g-head"><span class="term">capstones</span><span class="anchor">Fixtures/MultiAction/Honest/ActionCapstone.lean</span></div><div class="def">The top-level results a route ends at. The consensus-generic adaptive-statement knowledge capstone lets the adversary output the public statement and proof together, binds the VK and selected instance commitments before <code>theta</code>, exposes a Type-valued private-witness-or-relation branch, and bounds acceptance when that witness branch is absent by profiled DLOG advantage plus the statistical remainder. The currently justified profile gives <code>Adv_DLOG(2^127, 2^127) + 2^-83</code> at the <code>2^123</code> work target. This theorem does not by itself establish that the adaptive-statement witness projection is an executable extractor.</div></div>
