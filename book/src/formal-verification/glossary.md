...
<div class="g"><div class="g-head"><span class="term">high-level relation · VK correctness</span><span class="anchor">ActionCircuit.GateToActionBridge · Verifier.Assemble</span></div><div class="def">The two gaps the composition does not yet cross: on the output side, <code>GateToActionBridge</code> — the verifier's gate predicate decodes to the exact Clean Action assignment consumed by the proved circuit soundness theorem; on the input side, reusable VK correctness — the verifying key fed to the verifier faithfully encodes the real deployed circuit. <code>ActionCircuit.gate_hencodes</code> now proves the remaining last-mile composition once that representation bridge is supplied; the column recovery, full permutation/lookup semantics, and reusable VK theorem remain open.</div></div>
</section>
