...
<div class="g"><div class="g-head"><span class="term">expected extraction time</span><span class="anchor">Forking.Adversary.ExpectedRunsPoly.recursiveAlgebraicFork_oracle_tape_sum_runs_le_poly · AGM.ProbabilityCoins.relationWithCoins_prob_le_of_truncated_textbookDL</span></div><div class="def">The unconditional AFK analysis bounds the recursive extractor's expected adversary-run count, averaged over oracle tables and extractor tapes, by <code>(8·Q+1)·10ᵏ</code>. This is field-independent and does not assume <em>fork spread</em>; <code>reductionEfficient_poly</code> wires it into the family runtime interface. The deployed combined finder adds at most three calls, truncates at the fixed budget <code>L</code>, and charges the over-budget tail explicitly as <code>((8·Q+1)·10ᵏ+3)/(L+1)</code>. The earlier <code>(6/δ)ᵏ</code> bound under <code>FamilyForkSpread</code> remains as an optional density-sensitive alternative.</div></div>
<div class="g"><div class="g-head"><span class="term">DL reduction bound</span><span class="anchor">Forking.Adversary.Algebraic.ComputedAlgebraicFSFamily.binding_prob_le_of_generatorRO_textbookDL</span></div><div class="def">The programmed-basis accounting instantiated with the computed algebraic Fiat–Shamir producer. Binding is the computable mismatch between the effective accepted value — including the <code>z⁻¹·(multiU + ξ·sU)</code> shift from the adversary's declared <code>U</code> coefficients, zero for honest proofs — and the adversary's carried aggregate opening; never nonexistence of an opening, which holds vacuously at prime order. In the uniform generator-RO setup model, its probability is bounded by the recursive query loss, the <code>z = 0</code> slice, and the textbook-DL advantage plus <code>1/|Fp|</code>; a clean extracted opening on a mismatch run is converted to a commitment-collision relation rather than discarded. Knowledge-side duals (<code>snarkFailure_prob_le_of_*</code>) bound full acceptance without a clean opening across the same setup models, including the privately-randomized and arbitrary-domain adversaries; <code>knowledgeSoundness_under_DL</code>/<code>binding_under_DL</code> package them under per-family DL hardness (<code>DiscreteLogRelationHardFor</code>) gated by the extractor call bound.</div></div>
</section>
...
<div class="g"><div class="g-head"><span class="term">captured knowledge-error bound</span><span class="anchor">Fixture2.orchard_deployed_knowledge_error_captured</span></div><div class="def">The additive five-term bound on deployed extraction failure at the captured key: challenge pricing, the tight DLOG term <code>ε_DL + 1/|𝔽|</code>, the shape's root budget, and the concrete <code>(Q+1)·20470/|𝔽|</code> — no continuation threshold or fourth-root conversion anywhere. Named inputs: single-instance DLOG, the squeeze-pinning premises.</div></div>
</section>

<section>
<div class="grp">Capstones &amp; hypotheses</div>
<div class="g"><div class="g-head"><span class="term">capstones</span><span class="anchor">Soundness/Vesta.lean · Forking/Adversary/Algebraic.lean</span></div><div class="def">The top-level results a route ends at. One survives per route: the computed algebraic Fiat–Shamir endpoint carries representations through the adaptive adversary, recursively constructs the deployed instance, and bounds the modeled binding event (AGM adversary, ideal random oracle) by extraction loss plus plain DL. Earlier ladders — propositional opening and constraint forms across the conditional, deployed, adaptive, and rewind models — were retired once the rewind-free route subsumed them.</div></div>
