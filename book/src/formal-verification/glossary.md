...
<div class="g"><div class="g-head"><span class="term">high-level relation · VK correctness</span><span class="anchor">hencodes · Verifier.Assemble</span></div><div class="def">The two gaps the composition does not yet cross: on the output side, the decoded verifier columns and argument equations must produce the exact Clean Action assignment consumed by the proved circuit soundness theorem; on the input side, reusable VK correctness must show that the verifying key fed to the verifier faithfully encodes the real deployed circuit. The multiopen decode and full constraint-polynomial work narrow this bridge, but the Clean row environment, copy/lookup semantics, and reusable VK theorem remain open.</div></div>
</section>
...
<div class="g"><div class="g-head"><span class="term">checked trust boundary</span><span class="anchor">Zcash.TrustBoundary</span> (and temporarily other TrustBoundary modules)</div><div class="def">Build-time pins on what a theorem may rest on: <code>assert_axiom</code> asserts a bound on the axioms used by a definition, so that a stray <code>sorry</code> or a new axiom fails the build instead of silently widening the trusted base. <code>assert_computable</code> additionally asserts that the definition is a plain <code>def</code>, ensuring constructivity of security reductions. Until <a href="https://github.com/zcash/ironwood/issues/71">#71</a> is done, some of the TrustBoundary modules also use a <code>#guard_msgs</code>-pinned <code>#print axioms</code> check instead of these macros. See <a href="../formal-verification.html#trust-discipline">Trust discipline</a>.</div></div>
</section>
