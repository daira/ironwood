...
<div class="g"><div class="g-head"><span class="term">capstones</span><span class="anchor">Soundness/Vesta.lean · Composition/StraightLineConstraint.lean</span></div><div class="def">The top-level results a route ends at. One survives: the straight-line algebraic Fiat–Shamir endpoint carries representations through the adaptive adversary, decodes the deployed instance from a single accepting run, and bounds the modeled binding event (AGM adversary, ideal random oracle) by pinned root events plus plain DL. Earlier ladders — propositional opening and constraint forms across the conditional, deployed, adaptive, and rewind models — were retired once the rewind-free route subsumed them, and the rewinding extractor itself followed in ironwood#133.</div></div>
