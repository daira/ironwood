...
<div class="g"><div class="g-head"><span class="term">capstones</span><span class="anchor">Fixtures/MultiAction/Honest/ActionCapstone.lean</span></div><div class="def">The top-level results a route ends at. The consensus-generic adaptive-statement knowledge capstone lets the adversary output the public statement and proof together, binds the VK and selected instance commitments before <code>theta</code>, uses an executable Type-valued private-witness-or-relation branch, and bounds acceptance when witness extraction fails by profiled DLOG advantage plus the statistical remainder. The profile gives <code>Adv_DLOG(2^126, 2^126) + 2^-83</code> at the <code>2^123</code> computational work target. This is a 123-bit computational work-factor statement with an independent 83-bit statistical soundness error, not an unqualified 123-bit failure-probability claim.</div></div>
