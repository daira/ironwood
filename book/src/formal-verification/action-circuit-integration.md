...
selector freedom. Activation-row fit is now generic too: extracted lookup membership
...
| **[ME] lookup join** | Derive exact packed-selector zero/one values from the circuit-owned selector layout and fixed rows, then feed them to `TopLevelLookupWitnessConditions.ofChallengeExclusions`. | Correct-by-construction lookup lawfulness, generic routing/projection, activation-row fit, deployed-witness construction, bundle-wide bad-set aggregation, exact top-level `θ` pricing, and fixed commitment binding are complete. The remaining issue is stronger than gate activation: disabled selectors must evaluate to zero even when their packed column contains another selector's root. | The lookup field of `FullCircuitBridge` for every Action proof index. |
