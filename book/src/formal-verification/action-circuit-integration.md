...
3. **Current work:** derive exact packed-selector projection for lookups from the
   ...
   challenge packaging, and public-instance provenance are complete.
...
| **[ME] lookup join** | Derive exact packed-selector zero/one values for only the selector leaves occurring in each lookup input, then feed them to `TopLevelLookupWitnessConditions.ofChallengeExclusions`. | The former all-selectors premise was corrected because unrelated gate selectors may be active on the same row. `LookupSelectorRows` now proves the generic fixed-binding transport. Permanent compiler closure needs three missing laws: lookup inputs contain no simple selectors, selector kind is consistent by allocated index, and degree-zero selectors pack into singleton columns; V1 selector-column non-overlap then reduces the remaining zero facts to local activation membership. | The lookup field of `FullCircuitBridge` for every Action proof index. |
