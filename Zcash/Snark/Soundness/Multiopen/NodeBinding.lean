...
theorem deployed_aggregate_node_binding_of_x3consistency [DecidableEq G] [Inhabited G]
    ...
    (j : ℕ) (hj : j < deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch)
    {points evals : List Fp}
    (hlen : 0 < points.length)
    (hnode : Function.Injective (fun i : Fin points.length => points[i]))
    (hdeg : (openedDecodedCols pbatch ⟨deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch - 1 - j, by omega⟩).natDegree
      < points.length)
    (hconsistent : ∀ χ, OpenedX3Accept (instanceCommitment := instanceCommitment) urs hk vk ps ch b χ →
      (openedDecodedCols pbatch ⟨deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch - 1 - j, by omega⟩).eval χ
        = lagrangeEval χ points evals)
    {χ₀ : Fp} (hχ₀ : OpenedX3Accept (instanceCommitment := instanceCommitment) urs hk vk ps ch b χ₀)
    (hprob : (((points.length - 1 : ℕ)) : ℝ≥0∞) / Fintype.card Fp
      < (PMF.uniformOfFintype Fp).toOuterMeasure
          (Finset.univ.filter (OpenedX3Accept (instanceCommitment := instanceCommitment) urs hk vk ps ch b)))
    (i : Fin points.length) :
    (openedDecodedCols pbatch ⟨deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch - 1 - j, by omega⟩).eval points[i]
      = evals.getD (i : ℕ) 0 :=
  claimedEval_of_x3Prob hlen hnode hdeg (OpenedX3Accept (instanceCommitment := instanceCommitment) urs hk vk ps ch b) hconsistent hχ₀ hprob i
...
theorem openedX3_relation_of_accept [DecidableEq G] [Inhabited G]
    (urs : URS G) (hk : shape.k = urs.k) (vk : VerifyingKey shape Fp G)
    (ps : ProofString shape Fp G) (ch : Challenges shape.k Fp)
    {b : Fin (2 ^ urs.k) → Fp} {χ : Fp}
    (hacc : OpenedX3Accept (instanceCommitment := instanceCommitment) urs hk vk ps ch b χ) :
...
theorem openedX3_rewound_aggregate_value [DecidableEq G] [Inhabited G]
    (urs : URS G) (hk : shape.k = urs.k) (vk : VerifyingKey shape Fp G)
    (ps : ProofString shape Fp G) (ch : Challenges shape.k Fp) {χ : Fp} (j : ℕ)
    (hprob4 : ∀ r : X3Run shape G,
      (deployedX4PairCount (instanceCommitment := instanceCommitment) vk (r.spliced ps) (r.challenges ch χ) : ℝ≥0∞) / Fintype.card Fp
        < (PMF.uniformOfFintype Fp).toOuterMeasure (Finset.univ.filter
            (OpenedX4Accept (instanceCommitment := instanceCommitment) urs hk vk (r.spliced ps) (r.challenges ch χ) (evalVector urs.k χ))))
    (hacc : OpenedX3Accept (instanceCommitment := instanceCommitment) urs hk vk ps ch (evalVector urs.k χ) χ) :
    ∃ (r : X3Run shape G) (a : Fin (2 ^ urs.k) → Fp) (pU pW : Fp)
      (batch : OpenedBatchOpenings urs (evalVector urs.k χ)
        (x4BatchCommitments (instanceCommitment := instanceCommitment) urs hk vk (r.spliced ps) (r.challenges ch χ))
        (x4BatchEvals (instanceCommitment := instanceCommitment) vk (r.spliced ps) (r.challenges ch χ)) a pU pW),
      (openedDecodedCols batch
          ⟨deployedX4PairCount (instanceCommitment := instanceCommitment) vk (r.spliced ps) (r.challenges ch χ) - 1 - j, by omega⟩).eval χ
        = x4BatchEvals (instanceCommitment := instanceCommitment) vk (r.spliced ps) (r.challenges ch χ)
          ⟨deployedX4PairCount (instanceCommitment := instanceCommitment) vk (r.spliced ps) (r.challenges ch χ) - 1 - j, by omega⟩ := by
  obtain ⟨r, z, blind, fs, t, ht⟩ := hacc
  obtain ⟨a, ha⟩ := ipaRelation_extract urs (evalVector urs.k χ) fs.openedCommitment
    (multiopenValue vk instanceCommitment (r.spliced ps) (r.challenges ch χ)) t ht
  refine ⟨r, a, fs.pU, fs.pW,
    openedX4Rewind_of_x4Prob_forked (instanceCommitment := instanceCommitment) urs hk vk (r.spliced ps) (r.challenges ch χ) fs ⟨t, ht⟩
      (hprob4 r) a ha, ?_⟩
  exact openedDecodedCols_eval_x3 (instanceCommitment := instanceCommitment) urs hk vk (r.spliced ps) (r.challenges ch χ) _ _
...
value identity at `χ`. -/
theorem openedX3_agg_binding [DecidableEq G] [Inhabited G]
    (urs : URS G) (hk : shape.k = urs.k) (vk : VerifyingKey shape Fp G)
    (ps : ProofString shape Fp G) (ch : Challenges shape.k Fp) (r : X3Run shape G) {χ : Fp}
    (j : ℕ) (hj : j < deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch)
    {pU pW : Fp} {aa : Fin (2 ^ urs.k) → Fp}
    (pbatch : OpenedBatchOpenings urs (evalVector urs.k ch.x3)
      (x4BatchCommitments (instanceCommitment := instanceCommitment) urs hk vk ps ch) (x4BatchEvals (instanceCommitment := instanceCommitment) vk ps ch) aa pU pW)
    {aχ : Fin (2 ^ urs.k) → Fp} {pUχ pWχ : Fp}
    (batch_χ : OpenedBatchOpenings urs (evalVector urs.k χ)
      (x4BatchCommitments (instanceCommitment := instanceCommitment) urs hk vk (r.spliced ps) (r.challenges ch χ))
      (x4BatchEvals (instanceCommitment := instanceCommitment) vk (r.spliced ps) (r.challenges ch χ)) aχ pUχ pWχ) :
    openedDecodedCols pbatch ⟨deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch - 1 - j, by omega⟩
  ...
  have hcc : deployedX4PairCount (instanceCommitment := instanceCommitment) vk (r.spliced ps) (r.challenges ch χ)
      = deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch := x3Run_pairCount (instanceCommitment := instanceCommitment) vk ps ch r χ
  -- the two batches decode the same x₄-slot commitment
  have hcommeq : x4BatchCommitments (instanceCommitment := instanceCommitment) urs hk vk ps ch
...
theorem openedX3_perχ_consistency [DecidableEq G] [Inhabited G]
    (urs : URS G) (hk : shape.k = urs.k) (vk : VerifyingKey shape Fp G)
    (ps : ProofString shape Fp G) (ch : Challenges shape.k Fp) {χ : Fp} (j : ℕ)
    (hj : j < deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch)
    {pU pW : Fp} {aa : Fin (2 ^ urs.k) → Fp}
    (pbatch : OpenedBatchOpenings urs (evalVector urs.k ch.x3)
      (x4BatchCommitments (instanceCommitment := instanceCommitment) urs hk vk ps ch) (x4BatchEvals (instanceCommitment := instanceCommitment) vk ps ch) aa pU pW)
    {points evals : List Fp}
  ...
  obtain ⟨r, a, pUχ, pWχ, batch, hval⟩ :=
    openedX3_rewound_aggregate_value (instanceCommitment := instanceCommitment) urs hk vk ps ch j hprob4 hacc
  rcases openedX3_agg_binding (instanceCommitment := instanceCommitment) urs hk vk ps ch r j hj pbatch batch with heq | hdlr
...
theorem deployed_aggregate_node_binding_or_dlr [DecidableEq G] [Inhabited G]
    (urs : URS G) (hk : shape.k = urs.k) (vk : VerifyingKey shape Fp G)
    (ps : ProofString shape Fp G) (ch : Challenges shape.k Fp)
    {pU pW : Fp} {aa : Fin (2 ^ urs.k) → Fp}
    (pbatch : OpenedBatchOpenings urs (evalVector urs.k ch.x3)
    ...
    (hx2cons : ∀ χ : Fp, ∀ r : X3Run shape G,
      x4BatchEvals (instanceCommitment := instanceCommitment) vk (r.spliced ps) (r.challenges ch χ)
          ⟨deployedX4PairCount (instanceCommitment := instanceCommitment) vk (r.spliced ps) (r.challenges ch χ) - 1 - j, by omega⟩
        = lagrangeEval χ points evals)
    {χ₀ : Fp} (hχ₀ : OpenedX3Accept (instanceCommitment := instanceCommitment) urs hk vk ps ch (evalVector urs.k χ₀) χ₀)
    (hprob : (((points.length - 1 : ℕ)) : ℝ≥0∞) / Fintype.card Fp
      < (PMF.uniformOfFintype Fp).toOuterMeasure (Finset.univ.filter
          (fun χ => OpenedX3Accept (instanceCommitment := instanceCommitment) urs hk vk ps ch (evalVector urs.k χ) χ)))
    (i : Fin points.length) :
    (openedDecodedCols pbatch ⟨deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch - 1 - j, by omega⟩).eval points[i]
        = evals.getD (i : ℕ) 0
      ∨ HasNontrivialRelation (F := Fp) urs.g urs.u urs.w := by
  by_cases hcons : ∀ χ, OpenedX3Accept (instanceCommitment := instanceCommitment) urs hk vk ps ch (evalVector urs.k χ) χ →
      (openedDecodedCols pbatch ⟨deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch - 1 - j, by omega⟩).eval χ
        = lagrangeEval χ points evals
...
theorem hx2cons_slot_eq_multiopenU [DecidableEq G] [Inhabited G]
...
be stated. -/
noncomputable def deployedSetsForEval [DecidableEq G] [Inhabited G]
    (vk : VerifyingKey shape Fp G) (ps : ProofString shape Fp G) (ch : Challenges shape.k Fp) :
...
theorem deployedSetsForEval_length [DecidableEq G] [Inhabited G]
    (vk : VerifyingKey shape Fp G) (ps : ProofString shape Fp G) (ch : Challenges shape.k Fp) :
    (deployedSetsForEval (instanceCommitment := instanceCommitment) vk ps ch).length = deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch := by
...
theorem deployedSetsForEval_getD_points [DecidableEq G] [Inhabited G]
    (vk : VerifyingKey shape Fp G) (ps : ProofString shape Fp G) (ch : Challenges shape.k Fp)
    {k : ℕ} (hk : k < deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch) :
    ((deployedSetsForEval (instanceCommitment := instanceCommitment) vk ps ch).getD k ([], [], 0)).1
      = (constructIntermediateSets (assembleQueries vk instanceCommitment ps ch)).points.getD k [] := by
  have hlen := deployedSetsForEval_length (instanceCommitment := instanceCommitment) vk ps ch
  have hsp := constructIntermediateSets_points_length (assembleQueries vk instanceCommitment ps ch)
  have hcount : deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch
...
theorem deployedSetsForEval_getD_nodup [DecidableEq G] [Inhabited G]
    (vk : VerifyingKey shape Fp G) (ps : ProofString shape Fp G) (ch : Challenges shape.k Fp)
    {k : ℕ} (hk : k < deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch) :
    ((deployedSetsForEval (instanceCommitment := instanceCommitment) vk ps ch).getD k ([], [], 0)).1.Nodup := by
...
theorem deployedSetsForEval_reverse_getD_points [DecidableEq G] [Inhabited G]
    (vk : VerifyingKey shape Fp G) (ps : ProofString shape Fp G) (ch : Challenges shape.k Fp)
    {k : ℕ} (hk : k < deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch) :
    ((deployedSetsForEval (instanceCommitment := instanceCommitment) vk ps ch).reverse.getD k ([], [], 0)).1
      = (constructIntermediateSets (assembleQueries vk instanceCommitment ps ch)).points.getD
          (deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch - 1 - k) [] := by
  have hlen := deployedSetsForEval_length (instanceCommitment := instanceCommitment) vk ps ch
  have htup : (deployedSetsForEval (instanceCommitment := instanceCommitment) vk ps ch).reverse.getD k ([], [], 0)
      = (deployedSetsForEval (instanceCommitment := instanceCommitment) vk ps ch).getD (deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch - 1 - k) ([], [], 0) := by
    rw [List.getD_eq_getElem?_getD, List.getD_eq_getElem?_getD,
...
theorem deployedSetsForEval_reverse_getD_toFinset [DecidableEq G] [Inhabited G]
    (vk : VerifyingKey shape Fp G) (ps : ProofString shape Fp G) (ch : Challenges shape.k Fp)
    {k : ℕ} (hk : k < deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch) :
    ((deployedSetsForEval (instanceCommitment := instanceCommitment) vk ps ch).reverse.getD k ([], [], 0)).1.toFinset
...
theorem deployedSetsForEval_reverse_getD_nodup [DecidableEq G] [Inhabited G]
    (vk : VerifyingKey shape Fp G) (ps : ProofString shape Fp G) (ch : Challenges shape.k Fp)
    {k : ℕ} (hk : k < deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch) :
    ((deployedSetsForEval (instanceCommitment := instanceCommitment) vk ps ch).reverse.getD k ([], [], 0)).1.Nodup := by
...
theorem deployedSetsForEval_reverse_getD_u [DecidableEq G] [Inhabited G]
    (vk : VerifyingKey shape Fp G) (ps : ProofString shape Fp G) (ch : Challenges shape.k Fp)
    {k : ℕ} (hk : k < deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch) :
    ((deployedSetsForEval (instanceCommitment := instanceCommitment) vk ps ch).reverse.getD k ([], [], 0)).2.2
      = x4BatchEvals (instanceCommitment := instanceCommitment) vk ps ch ⟨k, Nat.lt_succ_of_lt hk⟩ := by
  have hlenS := deployedSetsForEval_length (instanceCommitment := instanceCommitment) vk ps ch
  have hlenP : (deployedX4Pairs (instanceCommitment := instanceCommitment) vk ps ch).length = deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch :=
    (deployedX4PairCount_eq (instanceCommitment := instanceCommitment) vk ps ch).symm
  have hik : deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch - 1 - k < deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch := by omega
  -- per-index: both the value-check set's u-field and the batch pair's eval are `multiopenU[i]`
  have key : ((deployedSetsForEval (instanceCommitment := instanceCommitment) vk ps ch).getD
        (deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch - 1 - k) ([], [], 0)).2.2
      = ((deployedX4Pairs (instanceCommitment := instanceCommitment) vk ps ch).getD
        (deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch - 1 - k) (Msm.zero shape.k Fp G, 0)).2 := by
  ...
  rw [List.getD_eq_getElem?_getD (l := (deployedSetsForEval (instanceCommitment := instanceCommitment) vk ps ch).reverse),
      List.getElem?_reverse (by rw [hlenS]; exact hk), hlenS,
      ← List.getD_eq_getElem?_getD,
      List.getD_eq_getElem?_getD (l := (deployedX4Pairs (instanceCommitment := instanceCommitment) vk ps ch).reverse),
      List.getElem?_reverse (by rw [hlenP]; exact hk), hlenP,
      ← List.getD_eq_getElem?_getD]
  exact key

attribute [local irreducible] deployedSetsForEval
...
theorem deployed_node_binding_of_grid [DecidableEq G] [Inhabited G]
    (urs : URS G) (hk : shape.k = urs.k) (vk : VerifyingKey shape Fp G)
    (ps : ProofString shape Fp G) (ch : Challenges shape.k Fp)
    ...
    (pbatch : OpenedBatchOpenings urs (evalVector urs.k ch.x3)
      (x4BatchCommitments (instanceCommitment := instanceCommitment) urs hk vk ps ch) (x4BatchEvals (instanceCommitment := instanceCommitment) vk ps ch) a₀ pU pW)
    (d : ℕ)
    (ζ : Fin (deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch) → Fp) (hζ : Function.Injective ζ)
    (χ : Fin (deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch) → Fin (d + 1) → Fp)
    (hχinj : ∀ s, Function.Injective (χ s))
    (qCol : Fin (deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch) → Polynomial Fp)
    (sets : Fin (deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch) → Fin (d + 1) → List (List Fp × List Fp × Fp))
    (hlen : ∀ s t, (sets s t).length = deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch)
    (hsetpts : ∀ s t (j : Fin (deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch)),
      ((sets s t).reverse.getD (j : ℕ) ([], [], 0)).1
        = ((deployedSetsForEval (instanceCommitment := instanceCommitment) vk ps ch).reverse.getD (j : ℕ) ([], [], 0)).1)
    (hsetevals : ∀ s t (j : Fin (deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch)),
      ((sets s t).reverse.getD (j : ℕ) ([], [], 0)).2.1
        = ((deployedSetsForEval (instanceCommitment := instanceCommitment) vk ps ch).reverse.getD (j : ℕ) ([], [], 0)).2.1)
    (hdeg : ∀ s, ((qCol s) * vanishingProd (deployedAllPts (instanceCommitment := instanceCommitment) vk ps ch)
        - ∑ j : Fin (deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch), C (ζ s ^ (j : ℕ)) *
            ((openedDecodedCols pbatch ⟨(j : ℕ), Nat.lt_succ_of_lt j.isLt⟩
              - lagrangePoly ((deployedSetsForEval (instanceCommitment := instanceCommitment) vk ps ch).reverse.getD (j : ℕ) ([], [], 0)).1
                  ((deployedSetsForEval (instanceCommitment := instanceCommitment) vk ps ch).reverse.getD (j : ℕ) ([], [], 0)).2.1)
              * coProd (deployedAllPts (instanceCommitment := instanceCommitment) vk ps ch)
                  (deployedSetPts (instanceCommitment := instanceCommitment) vk ps ch (deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch - 1 - (j : ℕ))))).natDegree
          ≤ d)
    (hnode : ∀ s t (j : Fin (deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch)),
      (vanishingProd (deployedSetPts (instanceCommitment := instanceCommitment) vk ps ch
        (deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch - 1 - (j : ℕ)))).eval (χ s t) ≠ 0)
    (hopen : ∀ s t, (qCol s).eval (χ s t) = multiopenEval (ζ s) (χ s t) (sets s t))
    (hu : ∀ s t (j : Fin (deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch)),
      (openedDecodedCols pbatch ⟨(j : ℕ), Nat.lt_succ_of_lt j.isLt⟩).eval (χ s t)
        = ((sets s t).reverse.getD (j : ℕ) ([], [], 0)).2.2)
    (j₀ : Fin (deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch)) {p : Fp}
    (hp : p ∈ deployedSetPts (instanceCommitment := instanceCommitment) vk ps ch (deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch - 1 - (j₀ : ℕ))) :
    (openedDecodedCols pbatch ⟨(j₀ : ℕ), Nat.lt_succ_of_lt j₀.isLt⟩).eval p
      = (lagrangePoly ((deployedSetsForEval (instanceCommitment := instanceCommitment) vk ps ch).reverse.getD (j₀ : ℕ) ([], [], 0)).1
          ((deployedSetsForEval (instanceCommitment := instanceCommitment) vk ps ch).reverse.getD (j₀ : ℕ) ([], [], 0)).2.1).eval p := by
  refine node_binding_of_grid_openings (deployedAllPts (instanceCommitment := instanceCommitment) vk ps ch)
    (fun j => deployedSetPts (instanceCommitment := instanceCommitment) vk ps ch (deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch - 1 - (j : ℕ)))
    (fun j => deployedSetPts_subset (instanceCommitment := instanceCommitment) vk ps ch _)
    (fun j => openedDecodedCols pbatch ⟨(j : ℕ), Nat.lt_succ_of_lt j.isLt⟩)
    (fun j => lagrangePoly ((deployedSetsForEval (instanceCommitment := instanceCommitment) vk ps ch).reverse.getD (j : ℕ) ([], [], 0)).1
        ((deployedSetsForEval (instanceCommitment := instanceCommitment) vk ps ch).reverse.getD (j : ℕ) ([], [], 0)).2.1)
    qCol ζ hζ d hdeg χ hχinj hnode sets hlen hopen ?_ ?_ hu ?_ j₀ hp
    (deployedSetPts_subset (instanceCommitment := instanceCommitment) vk ps ch _ hp)
  · -- hnd
    intro s t j
    rw [hsetpts s t j]; exact deployedSetsForEval_reverse_getD_nodup (instanceCommitment := instanceCommitment) vk ps ch j.isLt
  · -- hpts
    intro s t j
    rw [hsetpts s t j]; exact deployedSetsForEval_reverse_getD_toFinset (instanceCommitment := instanceCommitment) vk ps ch j.isLt
  · -- hr
    intro s t j
...
theorem openedX2_relation_of_accept [DecidableEq G] [Inhabited G]
    (urs : URS G) (hk : shape.k = urs.k) (vk : VerifyingKey shape Fp G)
    (ps : ProofString shape Fp G) (ch : Challenges shape.k Fp)
    {b : Fin (2 ^ urs.k) → Fp} {χ : Fp}
    (hacc : OpenedX2Accept (instanceCommitment := instanceCommitment) urs hk vk ps ch b χ) :
...
consumes as `hopen`. -/
theorem openedX3_rewound_qprime_value [DecidableEq G] [Inhabited G]
    (urs : URS G) (hk : shape.k = urs.k) (vk : VerifyingKey shape Fp G)
    ...
    ∃ (r : X3Run shape G) (a : Fin (2 ^ urs.k) → Fp) (pU pW : Fp)
      (batch : OpenedBatchOpenings urs (evalVector urs.k χ)
        ...
        (x4BatchEvals (instanceCommitment := instanceCommitment) vk (r.spliced ps) (r.challenges ch χ)) a pU pW),
      (openedDecodedCols batch
          ⟨deployedX4PairCount (instanceCommitment := instanceCommitment) vk (r.spliced ps) (r.challenges ch χ), Nat.lt_succ_self _⟩).eval χ
        = deployedBaseEval (instanceCommitment := instanceCommitment) vk (r.spliced ps) (r.challenges ch χ) := by
  obtain ⟨r, z, blind, fs, t, ht⟩ := hacc
  obtain ⟨a, ha⟩ := ipaRelation_extract urs (evalVector urs.k χ) fs.openedCommitment
    (multiopenValue vk instanceCommitment (r.spliced ps) (r.challenges ch χ)) t ht
  refine ⟨r, a, fs.pU, fs.pW,
    openedX4Rewind_of_x4Prob_forked (instanceCommitment := instanceCommitment) urs hk vk (r.spliced ps) (r.challenges ch χ) fs ⟨t, ht⟩
      (hprob4 r) a ha, ?_⟩
  exact openedDecodedCols_top_eval_x3 (instanceCommitment := instanceCommitment) urs hk vk (r.spliced ps) (r.challenges ch χ) _
...
theorem openedX2_agg_binding [DecidableEq G] [Inhabited G]
    (urs : URS G) (hk : shape.k = urs.k) (vk : VerifyingKey shape Fp G)
    ...
    openedDecodedCols pbatch ⟨deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch - 1 - j, by omega⟩
      ...
      ∨ HasNontrivialRelation (F := Fp) urs.g urs.u urs.w := by
  have hcc : deployedX4PairCount (instanceCommitment := instanceCommitment) vk (r.spliced ps) (r.challenges ch χ)
      = deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch := x2Run_pairCount (instanceCommitment := instanceCommitment) vk ps ch r χ
  have hcommeq : x4BatchCommitments (instanceCommitment := instanceCommitment) urs hk vk ps ch
        ⟨deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch - 1 - j, by omega⟩
      = x4BatchCommitments (instanceCommitment := instanceCommitment) urs hk vk (r.spliced ps) (r.challenges ch χ)
        ⟨deployedX4PairCount (instanceCommitment := instanceCommitment) vk (r.spliced ps) (r.challenges ch χ) - 1 - j, by omega⟩ := by
...
theorem openedX2X3_agg_binding [DecidableEq G] [Inhabited G]
    (urs : URS G) (hk : shape.k = urs.k) (vk : VerifyingKey shape Fp G)
    (ps : ProofString shape Fp G) (ch : Challenges shape.k Fp)
    ...
    {aχ : Fin (2 ^ urs.k) → Fp} {pUχ pWχ : Fp}
    (batch_χ : OpenedBatchOpenings urs (evalVector urs.k χ)
      (x4BatchCommitments (instanceCommitment := instanceCommitment) urs hk vk (r₃.spliced (r₂.spliced ps))
        (r₃.challenges (r₂.challenges ch ζ) χ))
      (x4BatchEvals (instanceCommitment := instanceCommitment) vk (r₃.spliced (r₂.spliced ps)) (r₃.challenges (r₂.challenges ch ζ) χ))
      aχ pUχ pWχ) :
    openedDecodedCols pbatch ⟨deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch - 1 - j, by omega⟩
        = openedDecodedCols batch_χ
      ...
      ∨ HasNontrivialRelation (F := Fp) urs.g urs.u urs.w := by
  have hcc : deployedX4PairCount (instanceCommitment := instanceCommitment) vk (r₃.spliced (r₂.spliced ps))
        (r₃.challenges (r₂.challenges ch ζ) χ) = deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch :=
    (x3Run_pairCount (instanceCommitment := instanceCommitment) vk (r₂.spliced ps) (r₂.challenges ch ζ) r₃ χ).trans
      (x2Run_pairCount (instanceCommitment := instanceCommitment) vk ps ch r₂ ζ)
  have hcommeq : x4BatchCommitments (instanceCommitment := instanceCommitment) urs hk vk ps ch
        ⟨deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch - 1 - j, by omega⟩
      = x4BatchCommitments (instanceCommitment := instanceCommitment) urs hk vk (r₃.spliced (r₂.spliced ps))
          (r₃.challenges (r₂.challenges ch ζ) χ)
...
theorem openedX3_qprime_binding [DecidableEq G] [Inhabited G]
    (urs : URS G) (hk : shape.k = urs.k) (vk : VerifyingKey shape Fp G)
    (ps : ProofString shape Fp G) (ch : Challenges shape.k Fp) (r : X3Run shape G) {χ : Fp}
    {pU pW : Fp} {aa : Fin (2 ^ urs.k) → Fp}
    (pbatch : OpenedBatchOpenings urs (evalVector urs.k ch.x3)
    ...
    (batch_χ : OpenedBatchOpenings urs (evalVector urs.k χ)
      (x4BatchCommitments (instanceCommitment := instanceCommitment) urs hk vk (r.spliced ps) (r.challenges ch χ))
      (x4BatchEvals (instanceCommitment := instanceCommitment) vk (r.spliced ps) (r.challenges ch χ)) aχ pUχ pWχ) :
    openedDecodedCols pbatch ⟨deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch, Nat.lt_succ_self _⟩
        = openedDecodedCols batch_χ
          ⟨deployedX4PairCount (instanceCommitment := instanceCommitment) vk (r.spliced ps) (r.challenges ch χ), Nat.lt_succ_self _⟩
      ∨ HasNontrivialRelation (F := Fp) urs.g urs.u urs.w := by
  have hcommeq : x4BatchCommitments (instanceCommitment := instanceCommitment) urs hk vk ps ch
        ⟨deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch, Nat.lt_succ_self _⟩
      = x4BatchCommitments (instanceCommitment := instanceCommitment) urs hk vk (r.spliced ps) (r.challenges ch χ)
        ⟨deployedX4PairCount (instanceCommitment := instanceCommitment) vk (r.spliced ps) (r.challenges ch χ), Nat.lt_succ_self _⟩ := by
...
theorem openedX3_qprime_binding_pair [DecidableEq G] [Inhabited G]
    (urs : URS G) (hk : shape.k = urs.k) (vk : VerifyingKey shape Fp G)
    (ps : ProofString shape Fp G) (ch : Challenges shape.k Fp)
    ...
    (batch : OpenedBatchOpenings urs (evalVector urs.k χ)
      (x4BatchCommitments (instanceCommitment := instanceCommitment) urs hk vk (r.spliced ps) (r.challenges ch χ))
      (x4BatchEvals (instanceCommitment := instanceCommitment) vk (r.spliced ps) (r.challenges ch χ)) aa pU pW)
    (batch' : OpenedBatchOpenings urs (evalVector urs.k χ')
      (x4BatchCommitments (instanceCommitment := instanceCommitment) urs hk vk (r'.spliced ps) (r'.challenges ch χ'))
      (x4BatchEvals (instanceCommitment := instanceCommitment) vk (r'.spliced ps) (r'.challenges ch χ')) aa' pU' pW') :
    openedDecodedCols batch
        ...
        = openedDecodedCols batch'
          ⟨deployedX4PairCount (instanceCommitment := instanceCommitment) vk (r'.spliced ps) (r'.challenges ch χ'), Nat.lt_succ_self _⟩
      ∨ HasNontrivialRelation (F := Fp) urs.g urs.u urs.w := by
  have hcommeq : x4BatchCommitments (instanceCommitment := instanceCommitment) urs hk vk (r.spliced ps) (r.challenges ch χ)
        ⟨deployedX4PairCount (instanceCommitment := instanceCommitment) vk (r.spliced ps) (r.challenges ch χ), Nat.lt_succ_self _⟩
      = x4BatchCommitments (instanceCommitment := instanceCommitment) urs hk vk (r'.spliced ps) (r'.challenges ch χ')
        ⟨deployedX4PairCount (instanceCommitment := instanceCommitment) vk (r'.spliced ps) (r'.challenges ch χ'), Nat.lt_succ_self _⟩ := by
    simp only [x4BatchCommitments, lt_self_iff_false]
...
theorem openedX3_rewound_batch_eval [DecidableEq G] [Inhabited G]
    (urs : URS G) (hk : shape.k = urs.k) (vk : VerifyingKey shape Fp G)
    (ps : ProofString shape Fp G) (ch : Challenges shape.k Fp) {χ : Fp}
    (hprob4 : ∀ r : X3Run shape G,
      (deployedX4PairCount (instanceCommitment := instanceCommitment) vk (r.spliced ps) (r.challenges ch χ) : ℝ≥0∞) / Fintype.card Fp
    ...
    ∃ (r : X3Run shape G) (a : Fin (2 ^ urs.k) → Fp) (pU pW : Fp)
      ...
      ∀ j, (openedDecodedCols batch j).eval χ
        = x4BatchEvals (instanceCommitment := instanceCommitment) vk (r.spliced ps) (r.challenges ch χ) j := by
  obtain ⟨r, z, blind, fs, t, ht⟩ := hacc
  obtain ⟨a, ha⟩ := ipaRelation_extract urs (evalVector urs.k χ) fs.openedCommitment
    (multiopenValue vk instanceCommitment (r.spliced ps) (r.challenges ch χ)) t ht
  refine ⟨r, a, fs.pU, fs.pW,
...
theorem deployedSetsForEval_x2x3_getD_fields [DecidableEq G] [Inhabited G]
    (vk : VerifyingKey shape Fp G) (ps : ProofString shape Fp G) (ch : Challenges shape.k Fp)
    (r₂ : X2Run shape G) (ζv : Fp) (r₃ : X3Run shape G) (χv : Fp) {k : ℕ}
    (hk : k < deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch) :
    ((deployedSetsForEval (instanceCommitment := instanceCommitment) vk (r₃.spliced (r₂.spliced ps))
        (r₃.challenges (r₂.challenges ch ζv) χv)).getD k ([], [], 0)).1
        = ((deployedSetsForEval (instanceCommitment := instanceCommitment) vk ps ch).getD k ([], [], 0)).1
      ∧ ((deployedSetsForEval (instanceCommitment := instanceCommitment) vk (r₃.spliced (r₂.spliced ps))
        (r₃.challenges (r₂.challenges ch ζv) χv)).getD k ([], [], 0)).2.1
        = ((deployedSetsForEval (instanceCommitment := instanceCommitment) vk ps ch).getD k ([], [], 0)).2.1 := by
  have hcc : deployedX4PairCount (instanceCommitment := instanceCommitment) vk (r₃.spliced (r₂.spliced ps))
      (r₃.challenges (r₂.challenges ch ζv) χv) = deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch :=
    (x3Run_pairCount (instanceCommitment := instanceCommitment) vk (r₂.spliced ps) (r₂.challenges ch ζv) r₃ χv).trans
      (x2Run_pairCount (instanceCommitment := instanceCommitment) vk ps ch r₂ ζv)
  have hk1 : k < (deployedSetsForEval (instanceCommitment := instanceCommitment) vk (r₃.spliced (r₂.spliced ps))
      (r₃.challenges (r₂.challenges ch ζv) χv)).length := by
    rw [deployedSetsForEval_length, hcc]; exact hk
  have hk2 : k < (deployedSetsForEval (instanceCommitment := instanceCommitment) vk ps ch).length := by
...
theorem deployedSetsForEval_x2x3_reverse_getD_fields [DecidableEq G] [Inhabited G]
    ...
    ((deployedSetsForEval (instanceCommitment := instanceCommitment) vk (r₃.spliced (r₂.spliced ps))
      ...
      ∧ ((deployedSetsForEval (instanceCommitment := instanceCommitment) vk (r₃.spliced (r₂.spliced ps))
        ...
        = ((deployedSetsForEval (instanceCommitment := instanceCommitment) vk ps ch).reverse.getD k ([], [], 0)).2.1 := by
  have hcc : deployedX4PairCount (instanceCommitment := instanceCommitment) vk (r₃.spliced (r₂.spliced ps))
      (r₃.challenges (r₂.challenges ch ζv) χv) = deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch :=
    (x3Run_pairCount (instanceCommitment := instanceCommitment) vk (r₂.spliced ps) (r₂.challenges ch ζv) r₃ χv).trans
      (x2Run_pairCount (instanceCommitment := instanceCommitment) vk ps ch r₂ ζv)
  have hlen1 : (deployedSetsForEval (instanceCommitment := instanceCommitment) vk (r₃.spliced (r₂.spliced ps))
      (r₃.challenges (r₂.challenges ch ζv) χv)).length = deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch := by
    rw [deployedSetsForEval_length, hcc]
  have hlen2 : (deployedSetsForEval (instanceCommitment := instanceCommitment) vk ps ch).length = deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch :=
    deployedSetsForEval_length (instanceCommitment := instanceCommitment) vk ps ch
  have h1 : (deployedSetsForEval (instanceCommitment := instanceCommitment) vk (r₃.spliced (r₂.spliced ps))
        (r₃.challenges (r₂.challenges ch ζv) χv)).reverse.getD k ([], [], 0)
      = (deployedSetsForEval (instanceCommitment := instanceCommitment) vk (r₃.spliced (r₂.spliced ps))
          (r₃.challenges (r₂.challenges ch ζv) χv)).getD
            (deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch - 1 - k) ([], [], 0) := by
    rw [List.getD_eq_getElem?_getD, List.getElem?_reverse (by rw [hlen1]; exact hk), hlen1,
      ← List.getD_eq_getElem?_getD]
  have h2 : (deployedSetsForEval (instanceCommitment := instanceCommitment) vk ps ch).reverse.getD k ([], [], 0)
      = (deployedSetsForEval (instanceCommitment := instanceCommitment) vk ps ch).getD (deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch - 1 - k) ([], [], 0) := by
    rw [List.getD_eq_getElem?_getD, List.getElem?_reverse (by rw [hlen2]; exact hk), hlen2,
      ← List.getD_eq_getElem?_getD]
  rw [h1, h2]
  exact deployedSetsForEval_x2x3_getD_fields (instanceCommitment := instanceCommitment) vk ps ch r₂ ζv r₃ χv (by omega)
...
theorem deployed_value_check_node_binding [DecidableEq G] [Inhabited G]
    (urs : URS G) (hk : shape.k = urs.k) (vk : VerifyingKey shape Fp G)
    (ps : ProofString shape Fp G) (ch : Challenges shape.k Fp)
    {a₀ : Fin (2 ^ urs.k) → Fp} {pU pW : Fp}
    (pbatch : OpenedBatchOpenings urs (evalVector urs.k ch.x3)
    ...
    (hζ₀ : OpenedX2Accept (instanceCommitment := instanceCommitment) urs hk vk ps ch b₂ ζ₀)
    (hprob2 : ((deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch - 1 : ℕ) : ℝ≥0∞) / Fintype.card Fp
      < (PMF.uniformOfFintype Fp).toOuterMeasure (Finset.univ.filter
          (OpenedX2Accept (instanceCommitment := instanceCommitment) urs hk vk ps ch b₂)))
    (hx3anchor : ∀ (r₂ : X2Run shape G) (ζv : Fp), ∃ χ₀ : Fp,
      OpenedX3Accept (instanceCommitment := instanceCommitment) urs hk vk (r₂.spliced ps) (r₂.challenges ch ζv) (evalVector urs.k χ₀) χ₀)
    (hprob3 : ∀ (r₂ : X2Run shape G) (ζv : Fp),
      ((max (2 ^ urs.k) (deployedAllPts (instanceCommitment := instanceCommitment) vk ps ch).card
        ...
        < (PMF.uniformOfFintype Fp).toOuterMeasure (Finset.univ.filter
            (fun χv => OpenedX3Accept (instanceCommitment := instanceCommitment) urs hk vk (r₂.spliced ps) (r₂.challenges ch ζv)
    ...
    (hprob4 : ∀ (r₂ : X2Run shape G) (ζv χv : Fp) (r₃ : X3Run shape G),
      (deployedX4PairCount (instanceCommitment := instanceCommitment) vk (r₃.spliced (r₂.spliced ps))
        ...
        < (PMF.uniformOfFintype Fp).toOuterMeasure (Finset.univ.filter
            (OpenedX4Accept (instanceCommitment := instanceCommitment) urs hk vk (r₃.spliced (r₂.spliced ps))
              (r₃.challenges (r₂.challenges ch ζv) χv) (evalVector urs.k χv))))
    (havoid : ∀ (r₂ : X2Run shape G) (ζv χv : Fp),
      OpenedX3Accept (instanceCommitment := instanceCommitment) urs hk vk (r₂.spliced ps) (r₂.challenges ch ζv) (evalVector urs.k χv) χv →
      ∀ k, χv ∉ deployedSetPts (instanceCommitment := instanceCommitment) vk ps ch k)
    (j₀ : Fin (deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch)) {p : Fp}
    (hp : p ∈ deployedSetPts (instanceCommitment := instanceCommitment) vk ps ch (deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch - 1 - (j₀ : ℕ))) :
    (openedDecodedCols pbatch ⟨(j₀ : ℕ), Nat.lt_succ_of_lt j₀.isLt⟩).eval p
        = (lagrangePoly ((deployedSetsForEval (instanceCommitment := instanceCommitment) vk ps ch).reverse.getD (j₀ : ℕ) ([], [], 0)).1
            ((deployedSetsForEval (instanceCommitment := instanceCommitment) vk ps ch).reverse.getD (j₀ : ℕ) ([], [], 0)).2.1).eval p
      ∨ HasNontrivialRelation (F := Fp) urs.g urs.u urs.w := by
  classical
  by_cases hrel : HasNontrivialRelation (F := Fp) urs.g urs.u urs.w
  · exact Or.inr hrel
  refine Or.inl ?_
  have hcpos : 0 < deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch := lt_of_le_of_lt (Nat.zero_le _) j₀.isLt
  have hn : deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch - 1 + 1 = deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch :=
    Nat.succ_pred_eq_of_pos hcpos
  -- the x₂ sample family, reindexed to `Fin count`
  have hζfam : ∃ ζ : Fin (deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch) → Fp,
      Function.Injective ζ ∧ ∀ s, OpenedX2Accept (instanceCommitment := instanceCommitment) urs hk vk ps ch b₂ (ζ s) := by
    obtain ⟨ζ', hinj, _, hacc⟩ := exists_injective_accepting_of_measure
      (acc := OpenedX2Accept (instanceCommitment := instanceCommitment) urs hk vk ps ch b₂) hζ₀ hprob2
  ...
  have hruns : ∀ s : Fin (deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch), ∃ r : X2Run shape G, True :=
    fun s => (hζacc s).elim fun r _ => ⟨r, trivial⟩
  choose r₂ hr₂ using hruns
  -- per-x₂ x₃ sample families at the rewound bases
  have hχfam : ∀ s : Fin (deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch),
      ∃ ξ : Fin (max (2 ^ urs.k) (deployedAllPts (instanceCommitment := instanceCommitment) vk ps ch).card
          + (deployedAllPts (instanceCommitment := instanceCommitment) vk ps ch).card + 1) → Fp,
        Function.Injective ξ ∧ ∀ t, OpenedX3Accept (instanceCommitment := instanceCommitment) urs hk vk ((r₂ s).spliced ps)
    ...
    obtain ⟨ξ, hinj, _, hacc⟩ := exists_injective_accepting_of_measure
      (acc := fun χv => OpenedX3Accept (instanceCommitment := instanceCommitment) urs hk vk ((r₂ s).spliced ps)
  ...
  have hbat : ∀ (s : Fin (deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch))
      (t : Fin (max (2 ^ urs.k) (deployedAllPts (instanceCommitment := instanceCommitment) vk ps ch).card
        + (deployedAllPts (instanceCommitment := instanceCommitment) vk ps ch).card + 1)),
      ∃ (r₃ : X3Run shape G) (a : Fin (2 ^ urs.k) → Fp) (pUχ pWχ : Fp)
        (B : OpenedBatchOpenings urs (evalVector urs.k (χ s t))
          (x4BatchCommitments (instanceCommitment := instanceCommitment) urs hk vk (r₃.spliced ((r₂ s).spliced ps))
            (r₃.challenges ((r₂ s).challenges ch (ζ s)) (χ s t)))
          (x4BatchEvals (instanceCommitment := instanceCommitment) vk (r₃.spliced ((r₂ s).spliced ps))
    ...
    fun s t => openedX3_rewound_batch_eval (instanceCommitment := instanceCommitment) urs hk vk ((r₂ s).spliced ps)
  ...
  have hccst : ∀ s t, deployedX4PairCount (instanceCommitment := instanceCommitment) vk ((r₃f s t).spliced ((r₂ s).spliced ps))
      ((r₃f s t).challenges ((r₂ s).challenges ch (ζ s)) (χ s t))
        = deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch := fun s t =>
    (x3Run_pairCount (instanceCommitment := instanceCommitment) vk ((r₂ s).spliced ps) ((r₂ s).challenges ch (ζ s)) (r₃f s t)
      (χ s t)).trans (x2Run_pairCount (instanceCommitment := instanceCommitment) vk ps ch (r₂ s) (ζ s))
  -- premise: value-check set lengths
  have hlen' : ∀ s t, (deployedSetsForEval (instanceCommitment := instanceCommitment) vk ((r₃f s t).spliced ((r₂ s).spliced ps))
      ((r₃f s t).challenges ((r₂ s).challenges ch (ζ s)) (χ s t))).length
        = deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch :=
    fun s t => (deployedSetsForEval_length (instanceCommitment := instanceCommitment) vk _ _).trans (hccst s t)
  -- premise: the grid runs' sets share the honest points and compressed evals
  have hsetpts' : ∀ s t (j : Fin (deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch)),
      ((deployedSetsForEval (instanceCommitment := instanceCommitment) vk ((r₃f s t).spliced ((r₂ s).spliced ps))
        ((r₃f s t).challenges ((r₂ s).challenges ch (ζ s)) (χ s t))).reverse.getD (j : ℕ)
          ([], [], 0)).1
        = ((deployedSetsForEval (instanceCommitment := instanceCommitment) vk ps ch).reverse.getD (j : ℕ) ([], [], 0)).1 :=
    fun s t j => (deployedSetsForEval_x2x3_reverse_getD_fields (instanceCommitment := instanceCommitment) vk ps ch (r₂ s) (ζ s)
      (r₃f s t) (χ s t) j.isLt).1
  have hsetevals' : ∀ s t (j : Fin (deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch)),
      ((deployedSetsForEval (instanceCommitment := instanceCommitment) vk ((r₃f s t).spliced ((r₂ s).spliced ps))
        ((r₃f s t).challenges ((r₂ s).challenges ch (ζ s)) (χ s t))).reverse.getD (j : ℕ)
          ([], [], 0)).2.1
        = ((deployedSetsForEval (instanceCommitment := instanceCommitment) vk ps ch).reverse.getD (j : ℕ) ([], [], 0)).2.1 :=
  ...
  have hdeg' : ∀ s, ((openedDecodedCols (Bf s 0)
      ⟨deployedX4PairCount (instanceCommitment := instanceCommitment) vk ((r₃f s 0).spliced ((r₂ s).spliced ps))
        ((r₃f s 0).challenges ((r₂ s).challenges ch (ζ s)) (χ s 0)), Nat.lt_succ_self _⟩)
        * vanishingProd (deployedAllPts (instanceCommitment := instanceCommitment) vk ps ch)
      - ∑ j : Fin (deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch), C (ζ s ^ (j : ℕ)) *
          ((openedDecodedCols pbatch ⟨(j : ℕ), Nat.lt_succ_of_lt j.isLt⟩
            - lagrangePoly ((deployedSetsForEval (instanceCommitment := instanceCommitment) vk ps ch).reverse.getD (j : ℕ) ([], [], 0)).1
                ((deployedSetsForEval (instanceCommitment := instanceCommitment) vk ps ch).reverse.getD (j : ℕ) ([], [], 0)).2.1)
            * coProd (deployedAllPts (instanceCommitment := instanceCommitment) vk ps ch)
                (deployedSetPts (instanceCommitment := instanceCommitment) vk ps ch
                  (deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch - 1 - (j : ℕ))))).natDegree
      ≤ max (2 ^ urs.k) (deployedAllPts (instanceCommitment := instanceCommitment) vk ps ch).card + (deployedAllPts (instanceCommitment := instanceCommitment) vk ps ch).card := by
    intro s
    refine grid_hdeg_bound (deployedAllPts (instanceCommitment := instanceCommitment) vk ps ch)
      (fun j => deployedSetPts (instanceCommitment := instanceCommitment) vk ps ch (deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch - 1 - (j : ℕ)))
      (fun j => openedDecodedCols pbatch ⟨(j : ℕ), Nat.lt_succ_of_lt j.isLt⟩)
      (fun j => lagrangePoly
        ((deployedSetsForEval (instanceCommitment := instanceCommitment) vk ps ch).reverse.getD (j : ℕ) ([], [], 0)).1
    ...
    · intro j
      ...
      · exact le_trans (le_of_lt (coeffsToPoly_natDegree_lt (by positivity) _))
          (le_max_left _ _)
      · have hnd := deployedSetsForEval_reverse_getD_nodup (instanceCommitment := instanceCommitment) vk ps ch j.isLt
        have hle := lagrangePoly_natDegree_le
          (points := ((deployedSetsForEval (instanceCommitment := instanceCommitment) vk ps ch).reverse.getD (j : ℕ) ([], [], 0)).1)
          (evals := ((deployedSetsForEval (instanceCommitment := instanceCommitment) vk ps ch).reverse.getD (j : ℕ) ([], [], 0)).2.1)
          (List.nodup_iff_injective_getElem.mp hnd)
        have hcard : ((deployedSetsForEval (instanceCommitment := instanceCommitment) vk ps ch).reverse.getD (j : ℕ) ([], [], 0)).1.length
            ≤ (deployedAllPts (instanceCommitment := instanceCommitment) vk ps ch).card := by
          rw [← List.toFinset_card_of_nodup hnd,
            deployedSetsForEval_reverse_getD_toFinset (instanceCommitment := instanceCommitment) vk ps ch j.isLt]
          exact Finset.card_le_card (deployedSetPts_subset (instanceCommitment := instanceCommitment) vk ps ch _)
        exact le_trans (le_trans hle hcard) (le_max_right _ _)
  -- premise: the samples avoid the nodes
  have hnode' : ∀ s t (j : Fin (deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch)),
      (vanishingProd (deployedSetPts (instanceCommitment := instanceCommitment) vk ps ch
        (deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch - 1 - (j : ℕ)))).eval (χ s t) ≠ 0 :=
  ...
  have hopen' : ∀ s t, (openedDecodedCols (Bf s 0)
      ...
      = multiopenEval (ζ s) (χ s t) (deployedSetsForEval
          (instanceCommitment := instanceCommitment) vk
          ((r₃f s t).spliced ((r₂ s).spliced ps))
          ((r₃f s t).challenges ((r₂ s).challenges ch (ζ s)) (χ s t))) := by
    intro s t
    rcases openedX3_qprime_binding_pair (instanceCommitment := instanceCommitment) urs hk vk ((r₂ s).spliced ps)
        ((r₂ s).challenges ch (ζ s)) (r₃f s 0) (r₃f s t) (Bf s 0) (Bf s t) with heq | hdlr
    · rw [heq, hBspec s t, x4BatchEvals_top (instanceCommitment := instanceCommitment)]
      exact deployedBaseEval_eq_multiopenEval (instanceCommitment := instanceCommitment) vk _ _
    · exact absurd hdlr hrel
  -- premise: the honest aggregate takes the grid runs' claimed set evals
  have hu' : ∀ s t (j : Fin (deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch)),
      (openedDecodedCols pbatch ⟨(j : ℕ), Nat.lt_succ_of_lt j.isLt⟩).eval (χ s t)
        = ((deployedSetsForEval (instanceCommitment := instanceCommitment) vk ((r₃f s t).spliced ((r₂ s).spliced ps))
            ((r₃f s t).challenges ((r₂ s).challenges ch (ζ s)) (χ s t))).reverse.getD (j : ℕ)
              ([], [], 0)).2.2 := by
    intro s t j
    have hjlt : deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch - 1 - (j : ℕ) < deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch := by
      have := j.isLt; omega
    rcases openedX2X3_agg_binding (instanceCommitment := instanceCommitment) urs hk vk ps ch (r₂ s) (r₃f s t)
        (deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch - 1 - (j : ℕ)) hjlt pbatch (Bf s t) with heq | hdlr
    swap
    · exact absurd hdlr hrel
    have hval1 : deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch - 1
        - (deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch - 1 - (j : ℕ)) = (j : ℕ) := by
      have := j.isLt; omega
    simp only [hval1] at heq
    rw [heq, hBspec s t]
    have hval2 : deployedX4PairCount (instanceCommitment := instanceCommitment) vk ((r₃f s t).spliced ((r₂ s).spliced ps))
      ...
      omega
    simp only [hval2]
    exact (deployedSetsForEval_reverse_getD_u (instanceCommitment := instanceCommitment) vk _ _
      (by rw [hccst s t]; exact j.isLt)).symm
  -- assemble
  exact deployed_node_binding_of_grid (instanceCommitment := instanceCommitment) urs hk vk ps ch pbatch
    (max (2 ^ urs.k) (deployedAllPts (instanceCommitment := instanceCommitment) vk ps ch).card + (deployedAllPts (instanceCommitment := instanceCommitment) vk ps ch).card)
    ζ hζinj χ hχinj
    (fun s => openedDecodedCols (Bf s 0)
      ⟨deployedX4PairCount (instanceCommitment := instanceCommitment) vk ((r₃f s 0).spliced ((r₂ s).spliced ps))
        ((r₃f s 0).challenges ((r₂ s).challenges ch (ζ s)) (χ s 0)), Nat.lt_succ_self _⟩)
    (fun s t => deployedSetsForEval (instanceCommitment := instanceCommitment) vk ((r₃f s t).spliced ((r₂ s).spliced ps))
      ((r₃f s t).challenges ((r₂ s).challenges ch (ζ s)) (χ s t)))
...
theorem deployedSetsForEval_getD_evals [DecidableEq G] [Inhabited G]
    (vk : VerifyingKey shape Fp G) (ps : ProofString shape Fp G) (ch : Challenges shape.k Fp)
    {k : ℕ} (hk : k < deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch) :
    ((deployedSetsForEval (instanceCommitment := instanceCommitment) vk ps ch).getD k ([], [], 0)).2.1
      = (compressSet ch.x1 (deployedSetQueries (instanceCommitment := instanceCommitment) vk ps ch k)
          ((constructIntermediateSets (assembleQueries vk instanceCommitment ps ch)).points.getD k []).length).2 := by
  have hsp := constructIntermediateSets_points_length (assembleQueries vk instanceCommitment ps ch)
  have hcnt : deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch
      ≤ (constructIntermediateSets (assembleQueries vk instanceCommitment ps ch)).points.length := by
    simp only [deployedX4PairCount, deployedX4Pairs, deployedX4Qs, List.length_map,
      List.length_zip, List.length_ofFn]
    omega
...
theorem x1Run_setPts [DecidableEq G] [Inhabited G]
    ...
    deployedSetPts (instanceCommitment := instanceCommitment) vk (r.spliced ps) (r.challenges ch ξ) k = deployedSetPts (instanceCommitment := instanceCommitment) vk ps ch k := by
...
theorem deployedSetsForEval_x1_getD_fields [DecidableEq G] [Inhabited G]
    (vk : VerifyingKey shape Fp G) (ps : ProofString shape Fp G) (ch : Challenges shape.k Fp)
    (r : X1Run shape G) (ξ : Fp) {k : ℕ} (hk : k < deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch) :
    ((deployedSetsForEval (instanceCommitment := instanceCommitment) vk (r.spliced ps) (r.challenges ch ξ)).getD k ([], [], 0)).1
        = (constructIntermediateSets (assembleQueries vk instanceCommitment ps ch)).points.getD k []
      ∧ ((deployedSetsForEval (instanceCommitment := instanceCommitment) vk (r.spliced ps) (r.challenges ch ξ)).getD k ([], [], 0)).2.1
        = (compressSet ξ (deployedSetQueries (instanceCommitment := instanceCommitment) vk ps ch k)
            ((constructIntermediateSets (assembleQueries vk instanceCommitment ps ch)).points.getD k []).length).2 := by
  have hcc : deployedX4PairCount (instanceCommitment := instanceCommitment) vk (r.spliced ps) (r.challenges ch ξ)
      = deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch := x1Run_pairCount (instanceCommitment := instanceCommitment) vk ps ch r ξ
  constructor
  · rw [deployedSetsForEval_getD_points (instanceCommitment := instanceCommitment) vk _ _ (by rw [hcc]; exact hk), x1Run_groupPoints]
  · rw [deployedSetsForEval_getD_evals (instanceCommitment := instanceCommitment) vk _ _ (by rw [hcc]; exact hk), x1Run_groupPoints,
      x1Run_setQueries]
    rfl
...
theorem openedX1_agg_member_binding [DecidableEq G] [Inhabited G]
    (urs : URS G) (hk : shape.k = urs.k) (vk : VerifyingKey shape Fp G)
    (ps : ProofString shape Fp G) (ch : Challenges shape.k Fp)
    ...
    (B : OpenedBatchOpenings urs bR
      (x4BatchCommitments (instanceCommitment := instanceCommitment) urs hk vk (r.spliced ps) (r.challenges ch ξ))
      (x4BatchEvals (instanceCommitment := instanceCommitment) vk (r.spliced ps) (r.challenges ch ξ)) aR pUR pWR) :
    (openedColumnDecode B).coeffs
      ...
      = ∑ m : Fin (deployedSetQueries (instanceCommitment := instanceCommitment) vk ps ch i).length, ξ ^ (m : ℕ) • md.cols m
    ∨ HasNontrivialRelation (F := Fp) urs.g urs.u urs.w := by
  have hcc : deployedX4PairCount (instanceCommitment := instanceCommitment) vk (r.spliced ps) (r.challenges ch ξ)
      = deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch := x1Run_pairCount (instanceCommitment := instanceCommitment) vk ps ch r ξ
  ...
  have hqs : i < (deployedX4Qs (instanceCommitment := instanceCommitment) vk (r.spliced ps) (r.challenges ch ξ)).length := by
    ...
    have hle : deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch ≤ (deployedX4Qs (instanceCommitment := instanceCommitment) vk ps ch).length := by
      simp only [deployedX4PairCount, deployedX4Pairs, List.length_zip, List.length_ofFn]
      omega
    omega
  rw [deployedX4Qs_getD_eval (instanceCommitment := instanceCommitment) (hk ▸ urs.g) urs.w urs.u vk (r.spliced ps) (r.challenges ch ξ) hqs,
  ...
  have hcomb : commit urs
        (∑ m : Fin (deployedSetQueries (instanceCommitment := instanceCommitment) vk ps ch i).length, ξ ^ (m : ℕ) • md.cols m)
      + (∑ m : Fin (deployedSetQueries (instanceCommitment := instanceCommitment) vk ps ch i).length, ξ ^ (m : ℕ) * md.uComp m) • urs.u
      + (∑ m : Fin (deployedSetQueries (instanceCommitment := instanceCommitment) vk ps ch i).length, ξ ^ (m : ℕ) * md.wComp m) • urs.w
      = ∑ m ∈ Finset.range (deployedSetQueries (instanceCommitment := instanceCommitment) vk ps ch i).length,
          ξ ^ m • ((deployedSetQueries (instanceCommitment := instanceCommitment) vk ps ch i).getD m (.point 0, [])).1.eval
            ⟨shape.k, hk ▸ urs.g, urs.w, urs.u⟩ := by
    rw [← Fin.sum_univ_eq_sum_range (fun m =>
      ξ ^ m • ((deployedSetQueries (instanceCommitment := instanceCommitment) vk ps ch i).getD m (.point 0, [])).1.eval
        ⟨shape.k, hk ▸ urs.g, urs.w, urs.u⟩)]
    have hsum : ∑ m : Fin (deployedSetQueries (instanceCommitment := instanceCommitment) vk ps ch i).length,
        ξ ^ (m : ℕ) • ((deployedSetQueries (instanceCommitment := instanceCommitment) vk ps ch i).getD (m : ℕ) (.point 0, [])).1.eval
          ⟨shape.k, hk ▸ urs.g, urs.w, urs.u⟩
...
theorem openedX1_agg_member_eval [DecidableEq G] [Inhabited G]
    (urs : URS G) (hk : shape.k = urs.k) (vk : VerifyingKey shape Fp G)
    (ps : ProofString shape Fp G) (ch : Challenges shape.k Fp)
    (i : ℕ) (hi : i < deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch)
    {a₀ : Fin (2 ^ urs.k) → Fp} {pU pW : Fp}
    ...
    (B : OpenedBatchOpenings urs bR
      (x4BatchCommitments (instanceCommitment := instanceCommitment) urs hk vk (r.spliced ps) (r.challenges ch ξ))
      (x4BatchEvals (instanceCommitment := instanceCommitment) vk (r.spliced ps) (r.challenges ch ξ)) aR pUR pWR) (p : Fp) :
    (openedDecodedCols B
        ⟨deployedX4PairCount (instanceCommitment := instanceCommitment) vk (r.spliced ps) (r.challenges ch ξ) - 1 - i, by omega⟩).eval p
      = ∑ m : Fin (deployedSetQueries (instanceCommitment := instanceCommitment) vk ps ch i).length,
          ξ ^ (m : ℕ) * (coeffsToPoly (md.cols m)).eval p
    ∨ HasNontrivialRelation (F := Fp) urs.g urs.u urs.w := by
  rcases openedX1_agg_member_binding (instanceCommitment := instanceCommitment) urs hk vk ps ch i hi md r ξ B with heq | hdlr
  · refine Or.inl ?_
...
theorem deployedSetQueries_eval_length [DecidableEq G] [Inhabited G]
    (vk : VerifyingKey shape Fp G) (ps : ProofString shape Fp G) (ch : Challenges shape.k Fp)
    (i : ℕ) :
    ∀ qc ∈ deployedSetQueries (instanceCommitment := instanceCommitment) vk ps ch i,
      qc.2.length
...
def OpenedX1PinnedAccept [DecidableEq G] [Inhabited G]  (urs : URS G)
  ...
  ∃ (run : X1Run shape G) (aR : Fin (2 ^ urs.k) → Fp) (pUR pWR : Fp),
    ...
    Nonempty (OpenedBatchOpenings urs (evalVector urs.k ((run.challenges ch χv).x3))
      (x4BatchCommitments (instanceCommitment := instanceCommitment) urs hk vk (run.spliced ps) (run.challenges ch χv))
      (x4BatchEvals (instanceCommitment := instanceCommitment) vk (run.spliced ps) (run.challenges ch χv)) aR pUR pWR)
...
theorem deployed_member_node_binding [DecidableEq G] [Inhabited G]
    (urs : URS G) (hk : shape.k = urs.k) (vk : VerifyingKey shape Fp G)
    (ps : ProofString shape Fp G) (ch : Challenges shape.k Fp)
    {a₀ : Fin (2 ^ urs.k) → Fp} {pU pW : Fp}
    {pbatch : OpenedBatchOpenings urs (evalVector urs.k ch.x3)
    ...
    (i : ℕ) (hi : i < deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch)
    (md : OpenedMemberDecode (instanceCommitment := instanceCommitment) urs hk vk ps ch pbatch i hi)
    {ξ₀ : Fp} (hξ₀ : OpenedX1PinnedAccept (instanceCommitment := instanceCommitment) urs hk vk ps ch ξ₀)
    (hprob1 : (((deployedSetQueries (instanceCommitment := instanceCommitment) vk ps ch i).length - 1 : ℕ) : ℝ≥0∞) / Fintype.card Fp
      < (PMF.uniformOfFintype Fp).toOuterMeasure (Finset.univ.filter
          (OpenedX1PinnedAccept (instanceCommitment := instanceCommitment) urs hk vk ps ch)))
    (hx2 : ∀ (r₁ : X1Run shape G) (ξv : Fp), ∃ (b₂ : Fin (2 ^ urs.k) → Fp) (ζ₀ : Fp),
      OpenedX2Accept (instanceCommitment := instanceCommitment) urs hk vk (r₁.spliced ps) (r₁.challenges ch ξv) b₂ ζ₀ ∧
      ((deployedX4PairCount (instanceCommitment := instanceCommitment) vk (r₁.spliced ps) (r₁.challenges ch ξv) - 1 : ℕ) : ℝ≥0∞)
          / Fintype.card Fp
        < (PMF.uniformOfFintype Fp).toOuterMeasure (Finset.univ.filter
            (OpenedX2Accept (instanceCommitment := instanceCommitment) urs hk vk (r₁.spliced ps) (r₁.challenges ch ξv) b₂)))
    (hx3anchor : ∀ (r₁ : X1Run shape G) (ξv : Fp) (r₂ : X2Run shape G) (ζv : Fp), ∃ χ₀ : Fp,
      OpenedX3Accept (instanceCommitment := instanceCommitment) urs hk vk (r₂.spliced (r₁.spliced ps))
        (r₂.challenges (r₁.challenges ch ξv) ζv) (evalVector urs.k χ₀) χ₀)
    (hprob3 : ∀ (r₁ : X1Run shape G) (ξv : Fp) (r₂ : X2Run shape G) (ζv : Fp),
      ((max (2 ^ urs.k) (deployedAllPts (instanceCommitment := instanceCommitment) vk (r₁.spliced ps) (r₁.challenges ch ξv)).card
          + (deployedAllPts (instanceCommitment := instanceCommitment) vk (r₁.spliced ps) (r₁.challenges ch ξv)).card : ℕ) : ℝ≥0∞)
          / Fintype.card Fp
        < (PMF.uniformOfFintype Fp).toOuterMeasure (Finset.univ.filter
            (fun χv => OpenedX3Accept (instanceCommitment := instanceCommitment) urs hk vk (r₂.spliced (r₁.spliced ps))
              (r₂.challenges (r₁.challenges ch ξv) ζv) (evalVector urs.k χv) χv)))
    (hprob4 : ∀ (r₁ : X1Run shape G) (ξv : Fp) (r₂ : X2Run shape G) (ζv χv : Fp)
        (r₃ : X3Run shape G),
      (deployedX4PairCount (instanceCommitment := instanceCommitment) vk (r₃.spliced (r₂.spliced (r₁.spliced ps)))
          (r₃.challenges (r₂.challenges (r₁.challenges ch ξv) ζv) χv) : ℝ≥0∞)
          / Fintype.card Fp
        < (PMF.uniformOfFintype Fp).toOuterMeasure (Finset.univ.filter
            (OpenedX4Accept (instanceCommitment := instanceCommitment) urs hk vk (r₃.spliced (r₂.spliced (r₁.spliced ps)))
              (r₃.challenges (r₂.challenges (r₁.challenges ch ξv) ζv) χv)
              (evalVector urs.k χv))))
    (havoid : ∀ (r₁ : X1Run shape G) (ξv : Fp) (r₂ : X2Run shape G) (ζv χv : Fp),
      OpenedX3Accept (instanceCommitment := instanceCommitment) urs hk vk (r₂.spliced (r₁.spliced ps))
        (r₂.challenges (r₁.challenges ch ξv) ζv) (evalVector urs.k χv) χv →
      ∀ k', χv ∉ deployedSetPts (instanceCommitment := instanceCommitment) vk (r₁.spliced ps) (r₁.challenges ch ξv) k')
    (hql : ∀ qc ∈ deployedSetQueries (instanceCommitment := instanceCommitment) vk ps ch i,
      qc.2.length
  ...
  · exact Or.inr hrel
  refine Or.inl ?_
  have hnpos : 0 < (deployedSetQueries (instanceCommitment := instanceCommitment) vk ps ch i).length :=
    lt_of_le_of_lt (Nat.zero_le _) m₀.isLt
  have hnn : (deployedSetQueries (instanceCommitment := instanceCommitment) vk ps ch i).length - 1 + 1
      = (deployedSetQueries (instanceCommitment := instanceCommitment) vk ps ch i).length := Nat.succ_pred_eq_of_pos hnpos
  -- the x₁ sample family
  have hξfam : ∃ ξ : Fin (deployedSetQueries (instanceCommitment := instanceCommitment) vk ps ch i).length → Fp,
      Function.Injective ξ ∧ ∀ s, OpenedX1PinnedAccept (instanceCommitment := instanceCommitment) urs hk vk ps ch (ξ s) := by
    obtain ⟨ξ', hinj, _, hacc⟩ := exists_injective_accepting_of_measure
      (acc := OpenedX1PinnedAccept (instanceCommitment := instanceCommitment) urs hk vk ps ch) hξ₀ hprob1
  ...
  have hruns : ∀ s : Fin (deployedSetQueries (instanceCommitment := instanceCommitment) vk ps ch i).length,
      ∃ (r₁ : X1Run shape G) (aR : Fin (2 ^ urs.k) → Fp) (pUR pWR : Fp),
        Nonempty (OpenedBatchOpenings urs (evalVector urs.k (((r₁.challenges ch (ξ s))).x3))
          (x4BatchCommitments (instanceCommitment := instanceCommitment) urs hk vk (r₁.spliced ps) (r₁.challenges ch (ξ s)))
          (x4BatchEvals (instanceCommitment := instanceCommitment) vk (r₁.spliced ps) (r₁.challenges ch (ξ s))) aR pUR pWR) := by
    intro s
  ...
  -- claimed evaluations
  have hagg : ∀ s : Fin (deployedSetQueries (instanceCommitment := instanceCommitment) vk ps ch i).length,
      ∑ m : Fin (deployedSetQueries (instanceCommitment := instanceCommitment) vk ps ch i).length,
          ξ s ^ (m : ℕ) * (coeffsToPoly (md.cols m)).eval
            (((constructIntermediateSets (assembleQueries vk instanceCommitment ps ch)).points.getD i [])[idx])
        = ∑ m : Fin (deployedSetQueries (instanceCommitment := instanceCommitment) vk ps ch i).length,
            ξ s ^ (m : ℕ)
              * ((deployedSetQueries (instanceCommitment := instanceCommitment) vk ps ch i).getD (m : ℕ) (.point 0, [])).2.getD (idx : ℕ) 0 := by
    intro s
    have hcc : deployedX4PairCount (instanceCommitment := instanceCommitment) vk ((r₁f s).spliced ps) ((r₁f s).challenges ch (ξ s))
        = deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch := x1Run_pairCount (instanceCommitment := instanceCommitment) vk ps ch (r₁f s) (ξ s)
    ...
    have hp_i : (((constructIntermediateSets (assembleQueries vk instanceCommitment ps ch)).points.getD i [])[idx])
        ∈ deployedSetPts (instanceCommitment := instanceCommitment) vk ((r₁f s).spliced ps) ((r₁f s).challenges ch (ξ s)) i := by
      rw [x1Run_setPts, deployedSetPts]
      exact List.mem_toFinset.mpr (List.getElem_mem idx.isLt)
    have hA := deployed_value_check_node_binding (instanceCommitment := instanceCommitment) urs hk vk ((r₁f s).spliced ps)
      ...
      ⟨deployedX4PairCount (instanceCommitment := instanceCommitment) vk ((r₁f s).spliced ps) ((r₁f s).challenges ch (ξ s)) - 1 - i,
        by omega⟩
      (by
        rw [show deployedX4PairCount (instanceCommitment := instanceCommitment) vk ((r₁f s).spliced ps) ((r₁f s).challenges ch (ξ s)) - 1
            - (deployedX4PairCount (instanceCommitment := instanceCommitment) vk ((r₁f s).spliced ps)
    ...
    have hrev : (deployedSetsForEval (instanceCommitment := instanceCommitment) vk ((r₁f s).spliced ps)
          ((r₁f s).challenges ch (ξ s))).reverse.getD
            (deployedX4PairCount (instanceCommitment := instanceCommitment) vk ((r₁f s).spliced ps)
              ((r₁f s).challenges ch (ξ s)) - 1 - i) ([], [], 0)
        = (deployedSetsForEval (instanceCommitment := instanceCommitment) vk ((r₁f s).spliced ps)
      ...
      rw [List.getD_eq_getElem?_getD,
        ...
        show deployedX4PairCount (instanceCommitment := instanceCommitment) vk ((r₁f s).spliced ps) ((r₁f s).challenges ch (ξ s)) - 1
          - (deployedX4PairCount (instanceCommitment := instanceCommitment) vk ((r₁f s).spliced ps)
              ((r₁f s).challenges ch (ξ s)) - 1 - i) = i from by omega,
        ← List.getD_eq_getElem?_getD]
    obtain ⟨hfpts, hfevals⟩ :=
      deployedSetsForEval_x1_getD_fields (instanceCommitment := instanceCommitment) vk ps ch (r₁f s) (ξ s) hi
    ...
    rw [compressSet_snd_getD (ξ s) (deployedSetQueries (instanceCommitment := instanceCommitment) vk ps ch i) (.point 0, [])
      idx.isLt hql] at hA
    -- the aggregate is the ξ-fold of the member values
    have hB := openedX1_agg_member_eval (instanceCommitment := instanceCommitment)
      (pbatch := pbatch)
      (bR := evalVector urs.k (((r₁f s).challenges ch (ξ s)).x3))
      (aR := aF s) (pUR := pUF s) (pWR := pWF s)
      urs hk vk ps ch i hi md (r₁f s) (ξ s) (hBne s).some
      (((constructIntermediateSets (assembleQueries vk instanceCommitment ps ch)).points.getD i [])[idx])
    rcases hB with hB | hdlr
    swap
    · exact absurd hdlr hrel
    rw [← hB, hA, ← Fin.sum_univ_eq_sum_range (fun m => ξ s ^ m
      * ((deployedSetQueries (instanceCommitment := instanceCommitment) vk ps ch i).getD m (.point 0, [])).2.getD (idx : ℕ) 0)]
  -- separate the members at the distinct samples
  exact member_binding_of_x1_samples (fun m => coeffsToPoly (md.cols m))
    (fun m => ((deployedSetQueries (instanceCommitment := instanceCommitment) vk ps ch i).getD (m : ℕ) (.point 0, [])).2.getD (idx : ℕ) 0)
    (((constructIntermediateSets (assembleQueries vk instanceCommitment ps ch)).points.getD i [])[idx])
    ξ hξinj hagg m₀
...
noncomputable def deployedClaimedFeed [DecidableEq G] [Inhabited G]
    (vk : VerifyingKey shape Fp G) (ps : ProofString shape Fp G) (ch : Challenges shape.k Fp)
    {num : ℕ} (setIdx : Fin num → ℕ)
    (memIdx : ∀ j : Fin num, Fin (deployedSetQueries (instanceCommitment := instanceCommitment) vk ps ch (setIdx j)).length)
    (layout : List (ℕ × ℤ)) : ℕ → Fp :=
  fun n =>
    if h : n < num then
      ((deployedSetQueries (instanceCommitment := instanceCommitment) vk ps ch (setIdx ⟨n, h⟩)).getD ((memIdx ⟨n, h⟩ : ℕ))
          (.point 0, [])).2.getD
        (((constructIntermediateSets (assembleQueries vk instanceCommitment ps ch)).points.getD
            (setIdx ⟨n, h⟩) []).idxOf
...
value binding. -/
theorem deployed_member_node_binding_at_point [DecidableEq G] [Inhabited G]
    (urs : URS G) (hk : shape.k = urs.k) (vk : VerifyingKey shape Fp G)
    (ps : ProofString shape Fp G) (ch : Challenges shape.k Fp)
    {a₀ : Fin (2 ^ urs.k) → Fp} {pU pW : Fp}
    {pbatch : OpenedBatchOpenings urs (evalVector urs.k ch.x3)
    ...
    (havoid : ∀ (r₁ : X1Run shape G) (ξv : Fp) (r₂ : X2Run shape G) (ζv χv : Fp),
      OpenedX3Accept (instanceCommitment := instanceCommitment) urs hk vk (r₂.spliced (r₁.spliced ps))
        (r₂.challenges (r₁.challenges ch ξv) ζv) (evalVector urs.k χv) χv →
      ∀ k', χv ∉ deployedSetPts (instanceCommitment := instanceCommitment) vk (r₁.spliced ps) (r₁.challenges ch ξv) k')
    {p : Fp} (hpt : p ∈ deployedSetPts (instanceCommitment := instanceCommitment) vk ps ch i)
    (m₀ : Fin (deployedSetQueries (instanceCommitment := instanceCommitment) vk ps ch i).length) :
    (coeffsToPoly (md.cols m₀)).eval p
      = ((deployedSetQueries (instanceCommitment := instanceCommitment) vk ps ch i).getD (m₀ : ℕ) (.point 0, [])).2.getD
  ...
  have hb := deployed_member_node_binding (instanceCommitment := instanceCommitment) urs hk vk ps ch i hi md hξ₀ hprob1 hx2 hx3anchor
    hprob3 hprob4 havoid (deployedSetQueries_eval_length (instanceCommitment := instanceCommitment) vk ps ch i) ⟨_, hlt⟩ m₀
  rcases hb with hb | hdlr
  · refine Or.inl ?_
