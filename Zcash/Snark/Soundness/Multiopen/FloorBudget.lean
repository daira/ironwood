...
`≤ deployedX4PairCount / |Fp|`, negligible. `OpenedX4Accept (instanceCommitment := instanceCommitment) urs hk vk ps ch b : Fp → Prop` is
...
own thresholds. -/
theorem openedX4_floor_failure_le [DecidableEq G] [Inhabited G]
    (urs : URS G) (hk : shape.k = urs.k) (vk : VerifyingKey shape Fp G)
    (ps : ProofString shape Fp G) (ch : Challenges shape.k Fp) (b : Fin (2 ^ urs.k) → Fp) :
    (PMF.uniformOfFintype Fp).toOuterMeasure
        {ξ : Fp | OpenedX4Accept (instanceCommitment := instanceCommitment) urs hk vk ps ch b ξ ∧
          ¬ ((deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch : ℝ≥0∞) / Fintype.card Fp
              < (PMF.uniformOfFintype Fp).toOuterMeasure
                  (Finset.univ.filter (OpenedX4Accept (instanceCommitment := instanceCommitment) urs hk vk ps ch b)))}
      ≤ (deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch : ℝ≥0∞) / Fintype.card Fp :=
  squeeze_floor_failure_le (OpenedX4Accept (instanceCommitment := instanceCommitment) urs hk vk ps ch b) _
...
theorem openedX3_floor_failure_le [DecidableEq G] [Inhabited G]
    (urs : URS G) (hk : shape.k = urs.k) (vk : VerifyingKey shape Fp G)
    (ps : ProofString shape Fp G) (ch : Challenges shape.k Fp) (b : Fin (2 ^ urs.k) → Fp)
    (t : ℝ≥0∞) :
    (PMF.uniformOfFintype Fp).toOuterMeasure
        {χ : Fp | OpenedX3Accept (instanceCommitment := instanceCommitment) urs hk vk ps ch b χ ∧
      ...
      ≤ t :=
  squeeze_floor_failure_le (OpenedX3Accept (instanceCommitment := instanceCommitment) urs hk vk ps ch b) t
...
theorem openedX2_floor_failure_le [DecidableEq G] [Inhabited G]
    (urs : URS G) (hk : shape.k = urs.k) (vk : VerifyingKey shape Fp G)
    (ps : ProofString shape Fp G) (ch : Challenges shape.k Fp) (b : Fin (2 ^ urs.k) → Fp)
    (t : ℝ≥0∞) :
    (PMF.uniformOfFintype Fp).toOuterMeasure
        {χ : Fp | OpenedX2Accept (instanceCommitment := instanceCommitment) urs hk vk ps ch b χ ∧
          ¬ (t < (PMF.uniformOfFintype Fp).toOuterMeasure
                  (Finset.univ.filter (OpenedX2Accept (instanceCommitment := instanceCommitment) urs hk vk ps ch b)))}
      ≤ t :=
  squeeze_floor_failure_le (OpenedX2Accept (instanceCommitment := instanceCommitment) urs hk vk ps ch b) t
...
`t := ((deployedSetQueries (instanceCommitment := instanceCommitment) vk ps ch i).length - 1)/|Fp|`, the per-set `hprob1` floor threshold).
Same template as `openedX4_floor_failure_le`. -/
theorem openedX1_floor_failure_le [DecidableEq G] [Inhabited G]
    (urs : URS G) (hk : shape.k = urs.k) (vk : VerifyingKey shape Fp G)
    (ps : ProofString shape Fp G) (ch : Challenges shape.k Fp) (t : ℝ≥0∞) :
    (PMF.uniformOfFintype Fp).toOuterMeasure
        {χ : Fp | OpenedX1Accept (instanceCommitment := instanceCommitment) urs hk vk ps ch χ ∧
          ¬ (t < (PMF.uniformOfFintype Fp).toOuterMeasure
                  (Finset.univ.filter (OpenedX1Accept (instanceCommitment := instanceCommitment) urs hk vk ps ch)))}
      ≤ t :=
  squeeze_floor_failure_le (OpenedX1Accept (instanceCommitment := instanceCommitment) urs hk vk ps ch) t
...
def deployedFloorFailureSet [DecidableEq G] [Inhabited G]
    (urs : URS G) (hk : shape.k = urs.k) (vk : VerifyingKey shape Fp G)
  ...
  {w : Fp × Fp × Fp × Fp | OpenedX1Accept (instanceCommitment := instanceCommitment) urs hk vk ps ch w.1 ∧
      ¬ (t₁ < (PMF.uniformOfFintype Fp).toOuterMeasure
              (Finset.univ.filter (OpenedX1Accept (instanceCommitment := instanceCommitment) urs hk vk ps ch)))}
    ∪ {w : Fp × Fp × Fp × Fp |
        OpenedX2Accept (instanceCommitment := instanceCommitment) urs hk vk ((g₁ w.1).spliced ps) ((g₁ w.1).challenges ch w.1)
            (b₂ w.1) w.2.1 ∧
      ¬ (t₂ < (PMF.uniformOfFintype Fp).toOuterMeasure
              (Finset.univ.filter (OpenedX2Accept (instanceCommitment := instanceCommitment) urs hk vk ((g₁ w.1).spliced ps)
                ((g₁ w.1).challenges ch w.1) (b₂ w.1))))}
    ∪ {w : Fp × Fp × Fp × Fp |
        OpenedX3Accept (instanceCommitment := instanceCommitment) urs hk vk ((g₂ w.1 w.2.1).spliced ((g₁ w.1).spliced ps))
            ((g₂ w.1 w.2.1).challenges ((g₁ w.1).challenges ch w.1) w.2.1)
            (evalVector urs.k w.2.2.1) w.2.2.1 ∧
      ¬ (t₃ < (PMF.uniformOfFintype Fp).toOuterMeasure
              (Finset.univ.filter (fun χv => OpenedX3Accept (instanceCommitment := instanceCommitment) urs hk vk
    ...
    ∪ {w : Fp × Fp × Fp × Fp |
      ...
      ¬ (t₄ < (PMF.uniformOfFintype Fp).toOuterMeasure
              (Finset.univ.filter (OpenedX4Accept (instanceCommitment := instanceCommitment) urs hk vk
...
theorem deployed_combined_floor_failure_le [DecidableEq G] [Inhabited G]
    (urs : URS G) (hk : shape.k = urs.k) (vk : VerifyingKey shape Fp G)
    (ps : ProofString shape Fp G) (ch : Challenges shape.k Fp)
    (g₁ : Fp → X1Run shape G) (g₂ : Fp → Fp → X2Run shape G) (g₃ : Fp → Fp → Fp → X3Run shape G)
    (b₂ : Fp → Fin (2 ^ urs.k) → Fp) (t₁ t₂ t₃ t₄ : ℝ≥0∞) :
    (PMF.uniformOfFintype (Fp × Fp × Fp × Fp)).toOuterMeasure
        (deployedFloorFailureSet (instanceCommitment := instanceCommitment) urs hk vk ps ch g₁ g₂ g₃ b₂ t₁ t₂ t₃ t₄)
      ≤ t₁ + t₂ + t₃ + t₄ :=
  combined_floor_failure_le
    (OpenedX1Accept (instanceCommitment := instanceCommitment) urs hk vk ps ch)
    (fun ξ ζ => OpenedX2Accept (instanceCommitment := instanceCommitment) urs hk vk ((g₁ ξ).spliced ps) ((g₁ ξ).challenges ch ξ) (b₂ ξ) ζ)
    (fun ξ ζ χ => OpenedX3Accept (instanceCommitment := instanceCommitment) urs hk vk ((g₂ ξ ζ).spliced ((g₁ ξ).spliced ps))
        ((g₂ ξ ζ).challenges ((g₁ ξ).challenges ch ξ) ζ) (evalVector urs.k χ) χ)
    (fun ξ ζ χ ω => OpenedX4Accept (instanceCommitment := instanceCommitment) urs hk vk
...
theorem deployed_combined_floor_holds [DecidableEq G] [Inhabited G]
    (urs : URS G) (hk : shape.k = urs.k) (vk : VerifyingKey shape Fp G)
    (ps : ProofString shape Fp G) (ch : Challenges shape.k Fp)
    (g₁ : Fp → X1Run shape G) (g₂ : Fp → Fp → X2Run shape G) (g₃ : Fp → Fp → Fp → X3Run shape G)
    (b₂ : Fp → Fin (2 ^ urs.k) → Fp) (t₁ t₂ t₃ t₄ : ℝ≥0∞) :
    1 - (t₁ + t₂ + t₃ + t₄) ≤
      (PMF.uniformOfFintype (Fp × Fp × Fp × Fp)).toOuterMeasure
        (deployedFloorFailureSet (instanceCommitment := instanceCommitment) urs hk vk ps ch g₁ g₂ g₃ b₂ t₁ t₂ t₃ t₄)ᶜ := by
  set μ := (PMF.uniformOfFintype (Fp × Fp × Fp × Fp)).toOuterMeasure with hμ
  set S := deployedFloorFailureSet (instanceCommitment := instanceCommitment) urs hk vk ps ch g₁ g₂ g₃ b₂ t₁ t₂ t₃ t₄ with hS
  have hfail : μ S ≤ t₁ + t₂ + t₃ + t₄ :=
    deployed_combined_floor_failure_le (instanceCommitment := instanceCommitment) urs hk vk ps ch g₁ g₂ g₃ b₂ t₁ t₂ t₃ t₄
...
decomposition. -/
theorem deployed_singlepath_floor_of_good [DecidableEq G] [Inhabited G]
    (urs : URS G) (hk : shape.k = urs.k) (vk : VerifyingKey shape Fp G)
    (ps : ProofString shape Fp G) (ch : Challenges shape.k Fp)
    (g₁ : Fp → X1Run shape G) (g₂ : Fp → Fp → X2Run shape G) (g₃ : Fp → Fp → Fp → X3Run shape G)
    (b₂ : Fp → Fin (2 ^ urs.k) → Fp) (t₁ t₂ t₃ t₄ : ℝ≥0∞)
    {w : Fp × Fp × Fp × Fp}
    (hw : w ∉ deployedFloorFailureSet (instanceCommitment := instanceCommitment) urs hk vk ps ch g₁ g₂ g₃ b₂ t₁ t₂ t₃ t₄) :
    (OpenedX1Accept (instanceCommitment := instanceCommitment) urs hk vk ps ch w.1 →
        t₁ < (PMF.uniformOfFintype Fp).toOuterMeasure
              (Finset.univ.filter (OpenedX1Accept (instanceCommitment := instanceCommitment) urs hk vk ps ch)))
    ∧ (OpenedX2Accept (instanceCommitment := instanceCommitment) urs hk vk ((g₁ w.1).spliced ps) ((g₁ w.1).challenges ch w.1) (b₂ w.1) w.2.1 →
        t₂ < (PMF.uniformOfFintype Fp).toOuterMeasure
              (Finset.univ.filter (OpenedX2Accept (instanceCommitment := instanceCommitment) urs hk vk ((g₁ w.1).spliced ps)
                ((g₁ w.1).challenges ch w.1) (b₂ w.1))))
    ∧ (OpenedX3Accept (instanceCommitment := instanceCommitment) urs hk vk ((g₂ w.1 w.2.1).spliced ((g₁ w.1).spliced ps))
