...
theorem openedDecodedCols_eval_x3 [DecidableEq G] [Inhabited G]
    ...
    (i : Fin (deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch + 1)) :
    (openedDecodedCols pbatch i).eval ch.x3 = x4BatchEvals (instanceCommitment := instanceCommitment) vk ps ch i := by
...
theorem openedDecodedCols_top_eval_x3 [DecidableEq G] [Inhabited G]
    (urs : URS G) (hk : shape.k = urs.k) (vk : VerifyingKey shape Fp G)
    (ps : ProofString shape Fp G) (ch : Challenges shape.k Fp)
    {a₀ : Fin (2 ^ urs.k) → Fp} {pU pW : Fp}
    (pbatch : OpenedBatchOpenings urs (evalVector urs.k ch.x3)
      (x4BatchCommitments (instanceCommitment := instanceCommitment) urs hk vk ps ch) (x4BatchEvals (instanceCommitment := instanceCommitment) vk ps ch) a₀ pU pW) :
    (openedDecodedCols pbatch ⟨deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch, Nat.lt_succ_self _⟩).eval ch.x3
      = deployedBaseEval (instanceCommitment := instanceCommitment) vk ps ch := by
  rw [openedDecodedCols_eval_x3]
  exact x4BatchEvals_top (instanceCommitment := instanceCommitment) vk ps ch
...
noncomputable def openedX4Batch_of_witnessFamily [DecidableEq G] [Inhabited G]
    (urs : URS G) (hk : shape.k = urs.k) (vk : VerifyingKey shape Fp G)
    ...
    (cur : Fin (deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch + 1))
    (aF : Fin (deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch + 1) → (Fin (2 ^ urs.k) → Fp))
    (pUF pWF : Fin (deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch + 1) → Fp)
    ...
    OpenedBatchOpenings urs b (x4BatchCommitments (instanceCommitment := instanceCommitment) urs hk vk ps ch) (x4BatchEvals (instanceCommitment := instanceCommitment) vk ps ch)
  ...
  commitment := fun r => by
    rw [hC r]
    exact deployedCommitment_x4_batch (instanceCommitment := instanceCommitment) urs hk vk ps ch (ξ r)
  value := fun r => by
    rw [hv r]
    exact multiopenValue_x4_batch (instanceCommitment := instanceCommitment) vk ps ch (ξ r)
...
def OpenedX4Accept [DecidableEq G] [Inhabited G]  (urs : URS G)
...
theorem openedX4Accept_of_deployedAccepts [DecidableEq G] [Inhabited G]
    (urs : URS G) (hk : shape.k = urs.k) (vk : VerifyingKey shape Fp G)
    (ps : ProofString shape Fp G) (ch : Challenges shape.k Fp)
    ...
    OpenedX4Accept (instanceCommitment := instanceCommitment) urs hk vk ps ch b ξv := by
  obtain ⟨fs, t, ht⟩ :=
    x4_cleanTree_of_deployedAccepts_adaptive (instanceCommitment := instanceCommitment) urs hk vk ps ch hz hnrel r ξv hFS hacc
...
the honest `{ch with x3 := χ}` value; the event therefore carries `multiopenValue vk instanceCommitment (r.spliced ps)
...
def OpenedX3Accept [DecidableEq G] [Inhabited G]  (urs : URS G)
    (hk : shape.k = urs.k) (vk : VerifyingKey shape Fp G) (ps : ProofString shape Fp G)
...
theorem openedX3Accept_of_deployedAccepts [DecidableEq G] [Inhabited G]
    (urs : URS G) (hk : shape.k = urs.k) (vk : VerifyingKey shape Fp G)
    (ps : ProofString shape Fp G) (ch : Challenges shape.k Fp)
    {b : Fin (2 ^ urs.k) → Fp} {z blind : Fp} (hz : z ≠ 0)
    (hnrel : ¬HasNontrivialRelation (F := Fp) urs.g urs.u urs.w)
    ...
    OpenedX3Accept (instanceCommitment := instanceCommitment) urs hk vk ps ch b χ := by
...
def OpenedX2Accept [DecidableEq G] [Inhabited G]  (urs : URS G)
    (hk : shape.k = urs.k) (vk : VerifyingKey shape Fp G) (ps : ProofString shape Fp G)
    (ch : Challenges shape.k Fp) (b : Fin (2 ^ urs.k) → Fp) (χ : Fp) : Prop :=
...
theorem openedX2Accept_of_deployedAccepts [DecidableEq G] [Inhabited G]
    (urs : URS G) (hk : shape.k = urs.k) (vk : VerifyingKey shape Fp G)
    (ps : ProofString shape Fp G) (ch : Challenges shape.k Fp)
    {b : Fin (2 ^ urs.k) → Fp} {z blind : Fp} (hz : z ≠ 0)
    (hnrel : ¬HasNontrivialRelation (F := Fp) urs.g urs.u urs.w)
    ...
    (hFS : FiatShamirTree urs hk vk instanceCommitment (r.spliced ps) (r.challenges ch χ) b z blind)
    (hacc : DeployedAccepts urs hk vk instanceCommitment (r.spliced ps) (r.challenges ch χ)) :
    OpenedX2Accept (instanceCommitment := instanceCommitment) urs hk vk ps ch b χ := by
  set fs := hFS (deployedAccepts_verifierEq urs hk vk instanceCommitment (r.spliced ps) _ hacc) with hfs
  rcases deployed_to_acceptV hz urs.g b fs.openedCommitment
      (multiopenValue vk instanceCommitment (r.spliced ps) (r.challenges ch χ)) blind fs.tree fs.accepts with
      hclean | hrel
...
noncomputable def openedX4Rewind_of_x4Prob [DecidableEq G] [Inhabited G]
    (urs : URS G) (hk : shape.k = urs.k) (vk : VerifyingKey shape Fp G)
    (ps : ProofString shape Fp G) (ch : Challenges shape.k Fp) {b : Fin (2 ^ urs.k) → Fp}
    (P : G) (pU pW : Fp)
    (hP : P = deployedCommitment urs hk vk instanceCommitment ps ch - pU • urs.u - pW • urs.w)
    (hx₀ : OpenedX4Accept (instanceCommitment := instanceCommitment) urs hk vk ps ch b ch.x4)
    (hprob4 : ((deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch : ℝ≥0∞)) / Fintype.card Fp
      < (PMF.uniformOfFintype Fp).toOuterMeasure (Finset.univ.filter
          (OpenedX4Accept (instanceCommitment := instanceCommitment) urs hk vk ps ch b))) :
    OpenedRewindForRelation urs P b (multiopenValue vk instanceCommitment ps ch)
      (x4BatchCommitments (instanceCommitment := instanceCommitment) urs hk vk ps ch) (x4BatchEvals (instanceCommitment := instanceCommitment) vk ps ch) pU pW := fun a hrel => by
  ...
  have hex := exists_injective_accepting_of_measure
    (acc := OpenedX4Accept (instanceCommitment := instanceCommitment) urs hk vk ps ch b) (x₀ := ch.x4) hx₀ hprob4
  set ξ := Classical.choose hex with hξdef
  have hspec := Classical.choose_spec hex
  have hξinj : Function.Injective ξ := hspec.1
  have hξ0 : ξ 0 = ch.x4 := hspec.2.1
  have hacc : ∀ r : Fin (deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch + 1),
  ...
  have hbatch := openedX4Batch_of_witnessFamily (instanceCommitment := instanceCommitment) urs hk vk ps ch ξ hξinj 0
...
noncomputable def openedX4Rewind_of_x4Prob_forked [DecidableEq G] [Inhabited G]
    (urs : URS G) (hk : shape.k = urs.k) (vk : VerifyingKey shape Fp G)
    (ps : ProofString shape Fp G) (ch : Challenges shape.k Fp) {b : Fin (2 ^ urs.k) → Fp}
    ...
    OpenedRewindForRelation urs fsCur.openedCommitment b (multiopenValue vk instanceCommitment ps ch)
      (x4BatchCommitments (instanceCommitment := instanceCommitment) urs hk vk ps ch) (x4BatchEvals (instanceCommitment := instanceCommitment) vk ps ch) fsCur.pU fsCur.pW :=
  openedX4Rewind_of_x4Prob (instanceCommitment := instanceCommitment) urs hk vk ps ch fsCur.openedCommitment fsCur.pU fsCur.pW rfl
...
theorem OpenedBatchOpenings.ipaRelation_of_x4Current [DecidableEq G] [Inhabited G]
    {urs : URS G} {hk : shape.k = urs.k} {vk : VerifyingKey shape Fp G}
    {ps : ProofString shape Fp G} {ch : Challenges shape.k Fp} {b : Fin (2 ^ urs.k) → Fp}
    {a₀ : Fin (2 ^ urs.k) → Fp} {pU pW : Fp}
    (hb : OpenedBatchOpenings urs b (x4BatchCommitments (instanceCommitment := instanceCommitment) urs hk vk ps ch)
  ...
  · have hc := hb.commitment hb.current
    ...
    have hd : (∑ j : Fin (deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch + 1),
        ch.x4 ^ (j : ℕ) • x4BatchCommitments (instanceCommitment := instanceCommitment) urs hk vk ps ch j)
        = deployedCommitment urs hk vk instanceCommitment ps ch :=
      (deployedCommitment_x4_batch (instanceCommitment := instanceCommitment) urs hk vk ps ch ch.x4).symm
    rw [hd] at hc
    rw [← hc]
    abel
  · have hv := hb.value hb.current
    ...
    have hd : (∑ j : Fin (deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch + 1),
        ch.x4 ^ (j : ℕ) • x4BatchEvals (instanceCommitment := instanceCommitment) vk ps ch j) = multiopenValue vk instanceCommitment ps ch :=
      (multiopenValue_x4_batch (instanceCommitment := instanceCommitment) vk ps ch ch.x4).symm
    rw [hd] at hv
...
theorem opened_witness_member_binding [DecidableEq G] [Inhabited G]
    (urs : URS G) (hk : shape.k = urs.k) (vk : VerifyingKey shape Fp G)
    (ps : ProofString shape Fp G) (ch : Challenges shape.k Fp)
    ...
    (wU wW : Fin (deployedSetQueries (instanceCommitment := instanceCommitment) vk ps ch i).length → Fp)
    ...
    ∧ ((openedColumnDecode pbatch).coeffs ⟨deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch - 1 - i, by omega⟩
        = ∑ m : Fin (deployedSetQueries (instanceCommitment := instanceCommitment) vk ps ch i).length,
            ch.x1 ^ (m : ℕ) • x1DecodeCols χ w m)
    ∧ ((openedColumnDecode pbatch).uComp ⟨deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch - 1 - i, by omega⟩
        = ∑ m : Fin (deployedSetQueries (instanceCommitment := instanceCommitment) vk ps ch i).length,
            ch.x1 ^ (m : ℕ) • x1DecodeComp χ wU m)
    ∧ ((openedColumnDecode pbatch).wComp ⟨deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch - 1 - i, by omega⟩
        = ∑ m : Fin (deployedSetQueries (instanceCommitment := instanceCommitment) vk ps ch i).length,
  ...
  have haC : ∀ r, commit urs (w r) + wU r • urs.u + wW r • urs.w
    ...
    intro r
    rw [hwC r,
      x1Run_x4Qs_getD_eval (instanceCommitment := instanceCommitment) (hk ▸ urs.g) urs.w urs.u vk ps ch (runs r) (χ r) hqs,
...
def OpenedX1Accept [DecidableEq G] [Inhabited G]  (urs : URS G)
    (hk : shape.k = urs.k) (vk : VerifyingKey shape Fp G) (ps : ProofString shape Fp G)
...
theorem openedX1Accept_of_deployedAccepts [DecidableEq G] [Inhabited G]
    (urs : URS G) (hk : shape.k = urs.k) (vk : VerifyingKey shape Fp G)
    ...
    (hopen : OpenedBatchOpenings urs bR
      (x4BatchCommitments (instanceCommitment := instanceCommitment) urs hk vk (run.spliced ps) (run.challenges ch χv))
      (x4BatchEvals (instanceCommitment := instanceCommitment) vk (run.spliced ps) (run.challenges ch χv)) aR pUR pWR) :
    OpenedX1Accept (instanceCommitment := instanceCommitment) urs hk vk ps ch χv :=
...
endpoint. -/
structure OpenedMemberDecode [DecidableEq G] [Inhabited G]
    (urs : URS G) (hk : shape.k = urs.k) (vk : VerifyingKey shape Fp G)
    (ps : ProofString shape Fp G) (ch : Challenges shape.k Fp)
    {b : Fin (2 ^ urs.k) → Fp} {a : Fin (2 ^ urs.k) → Fp} {pU pW : Fp}
    (pbatch : OpenedBatchOpenings urs b (x4BatchCommitments (instanceCommitment := instanceCommitment) urs hk vk ps ch)
      (x4BatchEvals (instanceCommitment := instanceCommitment) vk ps ch) a pU pW)
    (i : ℕ) (hi : i < deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch) where
  cols : Fin (deployedSetQueries (instanceCommitment := instanceCommitment) vk ps ch i).length → (Fin (2 ^ urs.k) → Fp)
  uComp : Fin (deployedSetQueries (instanceCommitment := instanceCommitment) vk ps ch i).length → Fp
  wComp : Fin (deployedSetQueries (instanceCommitment := instanceCommitment) vk ps ch i).length → Fp
  commitment : ∀ m : Fin (deployedSetQueries (instanceCommitment := instanceCommitment) vk ps ch i).length,
    commit urs (cols m) + uComp m • urs.u + wComp m • urs.w
      = ((deployedSetQueries (instanceCommitment := instanceCommitment) vk ps ch i).getD (m : ℕ) (.point 0, [])).1.eval
          ⟨shape.k, hk ▸ urs.g, urs.w, urs.u⟩
  reconstruct :
    (openedColumnDecode pbatch).coeffs ⟨deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch - 1 - i, by omega⟩
      = ∑ m : Fin (deployedSetQueries (instanceCommitment := instanceCommitment) vk ps ch i).length, ch.x1 ^ (m : ℕ) • cols m
  reconstructU :
    (openedColumnDecode pbatch).uComp ⟨deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch - 1 - i, by omega⟩
      = ∑ m : Fin (deployedSetQueries (instanceCommitment := instanceCommitment) vk ps ch i).length, ch.x1 ^ (m : ℕ) • uComp m
  reconstructW :
    (openedColumnDecode pbatch).wComp ⟨deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch - 1 - i, by omega⟩
      = ∑ m : Fin (deployedSetQueries (instanceCommitment := instanceCommitment) vk ps ch i).length, ch.x1 ^ (m : ℕ) • wComp m
...
noncomputable def openedMemberDecode_of_x1Prob [DecidableEq G] [Inhabited G]
    (urs : URS G) (hk : shape.k = urs.k) (vk : VerifyingKey shape Fp G)
    (ps : ProofString shape Fp G) (ch : Challenges shape.k Fp)
    {b : Fin (2 ^ urs.k) → Fp} {a : Fin (2 ^ urs.k) → Fp} {pU pW : Fp}
    (pbatch : OpenedBatchOpenings urs b (x4BatchCommitments (instanceCommitment := instanceCommitment) urs hk vk ps ch)
    ...
    OpenedMemberDecode (instanceCommitment := instanceCommitment) urs hk vk ps ch pbatch i hi := by
  classical
  have hcast : (deployedSetQueries (instanceCommitment := instanceCommitment) vk ps ch i).length - 1 + 1
      = (deployedSetQueries (instanceCommitment := instanceCommitment) vk ps ch i).length := Nat.succ_pred_eq_of_pos hlen
  ...
  have hex := exists_injective_accepting_of_measure
    (acc := OpenedX1Accept (instanceCommitment := instanceCommitment) urs hk vk ps ch) (x₀ := ch.x1) hx₁ hprob1
  set ξ₀ := Classical.choose hex with hξ₀def
  have hspec := Classical.choose_spec hex
  have hξinj₀ : Function.Injective ξ₀ := hspec.1
  have hξzero₀ : ξ₀ 0 = ch.x1 := hspec.2.1
  have hacc₀ : ∀ r, OpenedX1Accept (instanceCommitment := instanceCommitment) urs hk vk ps ch (ξ₀ r) := hspec.2.2
  -- Recast the rewound family to the member index type and pin the honest slot.
  set χ : Fin (deployedSetQueries (instanceCommitment := instanceCommitment) vk ps ch i).length → Fp :=
  ...
  have hcurv : χ ⟨0, hlen⟩ = ch.x1 := by
    have hzero : Fin.cast hcast.symm
        (⟨0, hlen⟩ : Fin (deployedSetQueries (instanceCommitment := instanceCommitment) vk ps ch i).length)
        = (0 : Fin ((deployedSetQueries (instanceCommitment := instanceCommitment) vk ps ch i).length - 1 + 1)) := by
      apply Fin.ext
      simp
    rw [hχdef]
    show ξ₀ (Fin.cast hcast.symm ⟨0, hlen⟩) = ch.x1
    rw [hzero, hξzero₀]
  have hacc : ∀ m : Fin (deployedSetQueries (instanceCommitment := instanceCommitment) vk ps ch i).length,
      OpenedX1Accept (instanceCommitment := instanceCommitment) urs hk vk ps ch (χ m) := fun m => hacc₀ _
  ...
  have hidx : ∀ m : Fin (deployedSetQueries (instanceCommitment := instanceCommitment) vk ps ch i).length,
      deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch - 1 - i
        < deployedX4PairCount (instanceCommitment := instanceCommitment) vk ((runsF m).spliced ps) ((runsF m).challenges ch (χ m)) + 1 := by
    intro m
    rw [x1Run_pairCount (instanceCommitment := instanceCommitment) vk ps ch (runsF m) (χ m)]
    omega
  -- The x₁ family fed to the member binding: choice data off the honest slot, the honest batch at it.
  set cur : Fin (deployedSetQueries (instanceCommitment := instanceCommitment) vk ps ch i).length := ⟨0, hlen⟩ with hcurdef
  set runs' : Fin (deployedSetQueries (instanceCommitment := instanceCommitment) vk ps ch i).length → X1Run shape G :=
    Function.update runsF cur (honestX1Run ps ch) with hrunsdef
  set w' : Fin (deployedSetQueries (instanceCommitment := instanceCommitment) vk ps ch i).length → (Fin (2 ^ urs.k) → Fp) :=
    Function.update
      (fun m => (openedColumnDecode (hpbne m).some).coeffs
        ⟨deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch - 1 - i, hidx m⟩)
      cur
      ((openedColumnDecode pbatch).coeffs
        ⟨deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch - 1 - i, by omega⟩) with hwdef
  set wU' : Fin (deployedSetQueries (instanceCommitment := instanceCommitment) vk ps ch i).length → Fp :=
    Function.update
      (fun m => (openedColumnDecode (hpbne m).some).uComp
        ⟨deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch - 1 - i, hidx m⟩)
      cur
      ((openedColumnDecode pbatch).uComp
        ⟨deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch - 1 - i, by omega⟩) with hwUdef
  set wW' : Fin (deployedSetQueries (instanceCommitment := instanceCommitment) vk ps ch i).length → Fp :=
    Function.update
      (fun m => (openedColumnDecode (hpbne m).some).wComp
        ⟨deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch - 1 - i, hidx m⟩)
      cur
      ((openedColumnDecode pbatch).wComp
        ⟨deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch - 1 - i, by omega⟩) with hwWdef
  have hIdxEq : deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch - 1 - (deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch - 1 - i)
      = i := by omega
  have hwC : ∀ r, commit urs (w' r) + wU' r • urs.u + wW' r • urs.w
    ...
    · subst hr
      ...
      have hj : ((⟨deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch - 1 - i, by omega⟩ :
          Fin (deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch + 1)) : ℕ) < deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch := by
        show deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch - 1 - i < deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch
        omega
      rw [(openedColumnDecode pbatch).commitment
          ⟨deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch - 1 - i, by omega⟩,
        x4BatchCommitments_getD (instanceCommitment := instanceCommitment) urs hk vk ps ch hj, hIdxEq, hcurv]
      rfl
    · rw [hwdef, hwUdef, hwWdef, hrunsdef]
      simp only [Function.update_of_ne hr]
      have hpc := x1Run_pairCount (instanceCommitment := instanceCommitment) vk ps ch (runsF r) (χ r)
      have hjR : ((⟨deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch - 1 - i, hidx r⟩ :
          Fin (deployedX4PairCount (instanceCommitment := instanceCommitment) vk ((runsF r).spliced ps)
            ((runsF r).challenges ch (χ r)) + 1)) : ℕ)
          < deployedX4PairCount (instanceCommitment := instanceCommitment) vk ((runsF r).spliced ps) ((runsF r).challenges ch (χ r)) := by
        show deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch - 1 - i
          < deployedX4PairCount (instanceCommitment := instanceCommitment) vk ((runsF r).spliced ps) ((runsF r).challenges ch (χ r))
        omega
      rw [(openedColumnDecode (hpbne r).some).commitment
          ⟨deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch - 1 - i, hidx r⟩,
        x4BatchCommitments_getD (instanceCommitment := instanceCommitment) urs hk vk ((runsF r).spliced ps)
          ((runsF r).challenges ch (χ r)) hjR]
      have hIdxR : deployedX4PairCount (instanceCommitment := instanceCommitment) vk ((runsF r).spliced ps) ((runsF r).challenges ch (χ r))
          - 1 - (deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch - 1 - i) = i := by
        omega
      rw [hIdxR]
  have hmb := opened_witness_member_binding (instanceCommitment := instanceCommitment) urs hk vk ps ch pbatch i hi χ hχinj cur hcurv
...
polynomials. -/
structure SnarkRelationWithMemberColumns [DecidableEq G] [Inhabited G]
    (urs : URS G) (hk : shape.k = urs.k) (vk : VerifyingKey shape Fp G)
    (ps : ProofString shape Fp G) (ch : Challenges shape.k Fp)
  ...
  opens : IpaRelation urs P b v a
  batchOpenings : OpenedBatchOpenings urs b (x4BatchCommitments (instanceCommitment := instanceCommitment) urs hk vk ps ch)
    (x4BatchEvals (instanceCommitment := instanceCommitment) vk ps ch) a pU pW
  ...
  adviceLayout : ∀ j : Fin numAdvice,
    (deployedSetCommIds (instanceCommitment := instanceCommitment) vk ps ch (adviceSet j)).getD (adviceMem j : ℕ) CommitmentId.vanishingH
  ...
  instanceLayout : ∀ j : Fin numInstance,
    (deployedSetCommIds (instanceCommitment := instanceCommitment) vk ps ch (instanceSet j)).getD (instanceMem j : ℕ) CommitmentId.vanishingH
      = CommitmentId.instanceCol p (vk.instanceQueryLayout.getD (j : ℕ) (0, 0)).1
  memberDecode : ∀ i (hi : i < deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch),
    OpenedMemberDecode (instanceCommitment := instanceCommitment) urs hk vk ps ch batchOpenings i hi
  ...
  quotCommitted : ∃ (hSet : ℕ) (hhSet : hSet < deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch)
      (hMem : Fin (deployedSetQueries (instanceCommitment := instanceCommitment) vk ps ch hSet).length),
    hpoly = coeffsToPoly ((memberDecode hSet hhSet).cols hMem) ∧
    (deployedSetCommIds (instanceCommitment := instanceCommitment) vk ps ch hSet).getD (hMem : ℕ) CommitmentId.randomPoly = CommitmentId.vanishingH
  satisfiesCircuit :
...
theorem member_constraint_of_relation_and_batch [DecidableEq G] [Inhabited G]
    (urs : URS G) (hk : shape.k = urs.k) (vk : VerifyingKey shape Fp G)
    (ps : ProofString shape Fp G) (ch : Challenges shape.k Fp)
    ...
    (pbatch : OpenedBatchOpenings urs b (x4BatchCommitments (instanceCommitment := instanceCommitment) urs hk vk ps ch)
      (x4BatchEvals (instanceCommitment := instanceCommitment) vk ps ch) a pU pW)
    (mdec : ∀ i (hi : i < deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch),
      OpenedMemberDecode (instanceCommitment := instanceCommitment) urs hk vk ps ch pbatch i hi)
    (hquot : quotientCheck
    ...
    (hinstanceLayout : ∀ j : Fin numInstance,
      (deployedSetCommIds (instanceCommitment := instanceCommitment) vk ps ch (instanceSet j)).getD (instanceMem j : ℕ) CommitmentId.vanishingH
        = CommitmentId.instanceCol p (vk.instanceQueryLayout.getD (j : ℕ) (0, 0)).1)
    (hquotCommitted : ∃ (hSet : ℕ) (hhSet : hSet < deployedX4PairCount (instanceCommitment := instanceCommitment) vk ps ch)
        (hMem : Fin (deployedSetQueries (instanceCommitment := instanceCommitment) vk ps ch hSet).length),
      hpoly = coeffsToPoly ((mdec hSet hhSet).cols hMem) ∧
      (deployedSetCommIds (instanceCommitment := instanceCommitment) vk ps ch hSet).getD (hMem : ℕ) CommitmentId.randomPoly
        = CommitmentId.vanishingH)
    {S : Prop}
    (hencodes : ∀ a,
      SnarkRelationWithMemberColumns (instanceCommitment := instanceCommitment) urs hk vk ps ch P b v p adviceSet hadviceSet adviceMem
...
theorem member_constraint_of_relation_and_batch_xgood [DecidableEq G] [Inhabited G]
    (urs : URS G) (hk : shape.k = urs.k) (vk : VerifyingKey shape Fp G)
    (ps : ProofString shape Fp G) (ch : Challenges shape.k Fp)
    {P : G} {b : Fin (2 ^ urs.k) → Fp} {v : Fp}
    {numAdvice numInstance : ℕ}
  ...
  exact member_constraint_of_relation_and_batch (instanceCommitment := instanceCommitment) urs hk vk ps ch adviceSet hadviceSet adviceMem
