...
theorem secretly rests on a `sorry` reached through some dependency, or on an unexpected axiom. Those are
...
* `#print axioms` pinned by `#guard_msgs` — freezes the exact axiom set `fingerprint_matches` rests on, so
  a newly introduced axiom (or a `sorryAx` from a stray `sorry`) changes the set and fails the build. The
