...
* `fingerprint_match_random_eval_sound` — soundness of the cheaper random-evaluation match: a
  ...
  (`Fixtures/*/MatchOnly/Epsilon.lean`; ε ≈ 2⁻²⁴⁰). The `native_decide` fixture match is what
...
* `perm_reindex_of_nodup_snd` / `msmMatch_other_reindex_of_nodup` — the `Perm`→positional
  ...
  random capture as `fingerprint_matches_positional` (`Fixtures/*/MatchOnly/Epsilon.lean`).
...
proofs; the three random families (`{SingleAction,MultiAction,TripleAction}/MatchOnly`) state it on random proof strings, so the slots whose honest values are
...
`dump_vesta_lean_fixture`) and three random match-only runs (the three
...
(`Fixtures/*/MatchOnly/Epsilon.lean`). -/
