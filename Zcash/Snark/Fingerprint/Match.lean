...
The capture APIs (`FingerprintStrategy`, `ChallengeRecorder`, `MSM::fingerprint_terms`,
`dump_vesta_lean_fixture`) have landed upstream and ship in released `halo2_proofs` 0.3.4
(originally [zcash/halo2#914](https://github.com/zcash/halo2/pull/914), with the capture test from
[zcash/orchard#531](https://github.com/zcash/orchard/pull/531)); `.github/workflows/fixtures.yml`
regenerates the fixtures in CI from the pinned release, so the capture path is no longer a draft
dependency. The captured fixture itself — the
