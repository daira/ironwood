...
[TalDerei/orchard#1](https://github.com/TalDerei/orchard/pull/1). The captured fixture itself — the
`2048` g-scalars, `100` commitment terms, `22` challenges, and the `VerifyingKey` — is loaded in
`Fixtures/SingleAction/Fixture.lean` (auto-generated by `dump_lean_fixture`), and `Fixture.fingerprint_matches`
runs the decidable comparison `MsmMatch (assemble vk ps ch) capturedMsm` by `native_decide`. The
group bases stay abstract — in the fixture they are opaque `ℕ` tags (`G := ℕ`), shared by
construction — so the match compares `F_p` scalars and says nothing about the Vesta curve: the
faithfulness "same `ℕ` tag ⇔ same Vesta point" lives in the Rust capture, not in Lean.
-/
