...
/-!
# Fiat–Shamir schedule check for the random three-action capture

The schedule checks of the honest families, restated at the random three-action capture: the
...
pins the Lean schedule model — the per-proof absorb interleaving at a bundle size no honest
fixture covers — at this generic input, and `nonInteractiveFingerprint_matches` folds the
schedule into the MSM match; `Boundary.lean` restates that match at the Lean-derived verifying
key as the family's statement of record.
-/
...
/-- The Lean schedule reaches every captured challenge for the random three-action proof. -/
