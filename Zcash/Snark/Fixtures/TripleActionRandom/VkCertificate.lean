...
/-!
# The random three-action verifying key, certified derived

The captured verifying key of the random three-action capture equals the key derived end-to-end
...
/-- **The captured random three-action verifying key is fully derived.** -/
theorem vk_eq_derived : vk = derivedVk := by
  unfold derivedVk
  rw [randomTriple_uses_same_urs]
  have hshape : actionCircuitShape_eq = Keygen.actionCircuitShape_eq_fixtureCircuitShape :=
    Subsingleton.elim _ _
  rw [hshape]
  exact randomTriple_uses_same_vk.trans Keygen.vk_eq_toVerifierKey
