...
non-group key profile, the schedule's `epsilonX` is the concrete `20470 / |𝔽|`. The causal
input is exact leave-one-`x` invariance; later unbatching challenges remain free.
-/
