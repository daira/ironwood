...
`snarkConstraintsDeployed_prob_le_of_root_schedule_runtime` bounds extraction failure for the
verifier's compressed constraint identity by six additive terms. This module instantiates it at the
captured Post-NU6.3 key: the static checks and degree caps come from the verifying key
(`StaticChecks`, `Schedule`), so the bad-`x` term is the concrete `(Q + 1) · 20470 / |𝔽|` and the
multiopen term is the shape's own root budget. No continuation threshold and no fourth root. The
combined finder is truncated at `L` black-box calls, and the unconditional AFK expectation
contributes the explicit tail `(afkRunBound Q k + 3)/(L+1)`.

Named inputs: fixed-call single-instance DLOG for the truncated combined finder (`hDL`), and the
two staged pre-squeeze traces exact pinning is derived from. The captured premise pins scalar
metadata, layouts, and expressions only — verifier commitments are AGM points carrying
representations over each sampled basis, so literal point equality is deliberately not required.
-/
...
theorem orchard_deployed_knowledge_error_captured
    ...
    {L : ℕ} {dlogBound : ENNReal}
    (hDL : TextbookDLWithCoinsTruncatedAdvantageLE B
      (deployedConstraintRelationFinder family.toRootFamily
        (deployedConstraintQuotientFinder family.toRootFamily))
      (deployedConstraintRelationFinderCalls family.toRootFamily) L dlogBound) :
    (independentProductPMF (orchardGeneratorROSetup query)
      ...
      ≤ ((family.Q + shape.k) * (3 / Fintype.card Fp) +
          ...
          (dlogBound + 1 / Fintype.card Fp +
            ((afkRunBound family.Q shape.k + 3 : ℕ) : ENNReal) / (L + 1 : ℕ)))
        + (family.Q + (11 + shape.k) + 1 : ℕ) * algebraicRootBudget shape shape.k
        + (family.Q + 1 : ℕ) * ((20470 : ℕ) / (Fintype.card Fp : ℝ≥0∞)) :=
  snarkConstraintsDeployed_prob_le_of_root_schedule_runtime B hB query hquery family.toRootFamily
    (deployedConstraintStaticChecks_of_captured family.toRootFamily hvk)
...
fixed-call single-instance DLOG for the truncated combined finder and the two staged chronology
traces. -/
    ...
    (hDL : TextbookDLWithCoinsTruncatedAdvantageLE B
      (deployedConstraintRelationFinder
        ...
        (deployedConstraintQuotientFinder
          (ComputedDeployedConstraintFSFamily.ofCovered online rootTrace xTrace).toRootFamily))
      (deployedConstraintRelationFinderCalls
        (ComputedDeployedConstraintFSFamily.ofCovered online rootTrace xTrace).toRootFamily)
      L dlogBound) :
    (independentProductPMF (orchardGeneratorROSetup query)
