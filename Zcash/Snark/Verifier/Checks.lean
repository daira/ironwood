...
This standalone helper is not on the assembly path: the live compression there fuses the commitment fold
with the per-point evaluation fold in `compressSet`. It is kept as an isolated building block. -/
