...
capture tooling on public maintainer-fork branches, pinned below by commit. Each head is
fixed exactly: the Halo2 pin is the current public #924 head descended from the 0.3.4
release, while the Orchard head is exactly one commit atop its release pin. The
regeneration script asserts both lineages. Nothing on either branch is deployed code:
the branch commits extend the dev/test capture tooling only, and
...
|---|---|---|
| `zcash/halo2` PR #924 (`ebfull/halo2`, branch `fingerprint-random-capture`) | `091e54786009f23c49a0531cbb980b8d05f7099f` | descends from `c12e83c4854aaa841bfd2ebc2451936a15a1cfb3` — `zcash/halo2` branch `release/halo2_proofs-0.3.4`. The pinned PR head includes the reviewed capture-tooling follow-ups merged into #924. The published halo2_proofs 0.3.4 crate (crates.io checksum `5f63a999d9223fa9d3b9db3031fedc316e6039a0ae0f67394408b16ef670c69f`) has a byte-identical `src/` tree at the release base |
...
`git diff c12e83c4..091e5478` (halo2) and `git diff ae351107..cec7031f` (orchard), or via
GitHub's compare view between the pinned base and the branch head.

### The capture changes

The pinned Halo2 #924 head combines the following reviewed source changes:
...
**halo2 #2 — "chore: make the exported fixture claim only what was captured"** —
...
**halo2 #3 — "chore: more 924 followups"** — makes fixture-mode dispatch
...
**halo2 #4 — "chore: more 924 follow-ups"** — documents and locally permits the shared
...
checkout paths for a fully offline run), asserts the Halo2 release ancestry and Orchard's
exact one-commit lineage, runs all five capture drivers in one pass, and diffs every
generated artifact against the committed one. All five fixture families are committed under
`Zcash/Snark/Fixtures/`, so
