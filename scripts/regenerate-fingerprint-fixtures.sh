...
# fabricate→replay random-capture drivers in orchard). The Halo2 pin is the exact public
# PR #924 head descended from its release pin; the Orchard head is exactly one commit atop
# its release pin. Both lineages are asserted below. This script proves both claims at once:
#
...
# The capture-tooling branch heads and release commits used by CI's honest regeneration
# (fixtures.yml).
